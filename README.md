...
| `tools/daily_log_review.py` | **日次ログレビュー（AIフィードバック）**（daily_log_review.yml、平日0:30 JST）。GitHub REST API（`gh api .../actions/runs`）で直近24hの全ワークフロー（ci.yml と自分自身を除く。ファイル`path`で束ねるので ops.yml の Keepalive/Watchdog のようにジョブ名で表示される run も漏れない）の実行結果とログを取得し、エラー・警告・記事タイトル・投稿/見送り行と各ステップの先頭・末尾だけに圧縮（`condense_log`）してClaude（claude-opus-5）に渡す。SRE／クオンツ運用／プロダクト／SEO・GEO（生成AI検索最適化）の4観点で「健全性サマリー／異常・注意点／SEO・GEO所見／改善提案（優先度付き）／LINE要約」をMarkdownで書かせ、全文は`daily-review`ラベルのIssue1本にコメント追記＋Actionsジョブサマリー、「LINE要約」節だけをLINEへpush。`--dry-run`で標準出力のみ、`--since-hours N`で対象期間変更。1回あたり入力10〜20万字・API費用は概ね$0.5前後 |
| `tests/test_daily_log_review.py` | 日次ログレビューのユニットテスト（ログ圧縮の残す行/落とす行・サイズ上限・run絞り込み・path別グルーピング・LINE要約抽出・メッセージ組み立て、9件）|
