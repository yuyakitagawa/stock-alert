...
- `web/generate_descriptions.py`：未生成の銘柄を Claude Haiku で一括生成（社名＋業種ベース、レート上限対策で
  20件/リクエストのバッチ＋429リトライ）→ Supabase `ai_analyses`(model_version=`company-desc-v1`) に保存。
  日次パイプライン Step 5a2 で差分補完（`--limit` で1回の上限）。`--all` で全件生成。
...
| `web/generate_descriptions.py` | 全銘柄の会社説明をClaude Haikuで一括生成（20件/リクエストのバッチ＋429リトライ・差分のみ）→ `ai_analyses`(company-desc-v1)。Step 5a2 |
