...
（TDnet自社株買い決定の記事化。上限10億円以上 or 発行済3%以上）。株価更新パイプライン(daily_alert.yml)から完全に独立した
...
ops.yml（運用系3本立て。平日06:00 UTC=keepalive空コミット／平日13:00 UTC=heartbeat=`tools/output_heartbeat.py`で当日の成果物（ブログ・X・動画）と素材（EDINET APIから直接）を数え欠けていればLINE通知／平日14:30 UTC=watchdog=daily_alert.ymlが今日成功していなければ再実行。`github.event.schedule`でジョブを分岐し、手動実行は`job`入力で選ぶ）、
...
各ワークフロー（daily_alert / edinet_blog / video_post / x_post / daily_log_review / ops-heartbeat）には
...
| `lib/writing_style.py` | **AI生成文の「機械っぽさ」対策の共通文体ルール**。ブログ記事（publish_blog_articles/publish_buyback_articles）・動画ナレーション（build_script）・英訳（translate_blog_articles_en）のプロンプトに埋め込む禁止表現ルール（`JA_STYLE_RULES`/`EN_STYLE_RULES`/`NARRATION_STYLE_RULES`: 「注目が集まっています」「〜と言えるでしょう」等の常套句・同一文末の連続・接続詞頼みの文つなぎを禁止）と、生成後にAI常套句・単調文末（同一文末4連続）を検出する`find_ai_tells()`を集約。検出時は記事生成側が1回だけ再生成し、字数充足 > 常套句の少なさ > 字数の優先度でマシな方を採用する（`body_quality_key()`）|
...
| `lib/buyback.py` | **自社株買い開示の分類・取得枠抽出**。`ext_tdnet_disclosures`（category=自社株買い）のタイトルで「決定」（取得枠の新設・ToSTNeT-3買付）・「進捗」（月次の取得状況報告）・**「変更/訂正/中止」（`_AMENDMENT_RE`。既存の取得枠に対する事後の修正）**を分け、決定開示の原文PDF（TDnet）を`pypdf`でテキスト化して上限株数・上限金額・発行済比率・取得期間・方法・消却有無を正規表現で抽出（取れない開示のみClaude Haikuで補助）し`tdnet_buybacks`に保存。**変更/訂正/中止は決定判定より先に落とす**（`_DECISION_RE`の「取得に係る事項」が「自己株式取得に係る事項の一部変更」にも当たるため決定と誤判定されていた。実例: 8560が2026-08-18に出した一部変更は取締役会決議が2026-02-09なのに上限11億円・発行済9.12%で記事化の閾値を超えており、開示日に新規決議したという体裁の記事が公開されるところだった＝2026-08-27修正。記事化されずに済んでいたのは`jpx_stock_list`に銘柄名が無く偶然スキップされていたためで、ロジックでは防げていなかった。中止はニュース価値があるが本文テンプレートが新規決議前提のため同じ記事型では出さない。落とした開示は`pending_decisions`が件数とタイトルをログに出す）。TDnetのPDFは公開から約1ヶ月で404になるため日次で回す。kujira-watch側は `src/lib/buybacks.ts` が `/stocks/[code]` の履歴表と TOPタブ `/buybacks`（直近30日の決定を開示日の新しい順に一覧、FAQ）で参照 |
...
| `web/x_buyback.py` | **平日「本日の自社株買い決定」のX投稿**（x_post.yml、平日19:00 JST）。`tdnet_buybacks`の当日分（JST）を上限金額順に並べ、上限1億円未満は除外。自社株買いは引け後（15:30〜17:00）に集中するため、投稿前に`lib/tdnet.scan_disclosures`と`lib/buyback.enrich`を回して当日分を取り込む。カード画像は`x_card_image.build_list_card` |
| `web/publish_buyback_articles.py` | **自社株買い決定のブログ記事自動生成・投稿**（edinet_blog.yml、`publish_blog_articles.py`の後段）。上限10億円以上 or 発行済3%以上の決定開示を対象に、事実のみからHaikuで本文（ja/en）を生成。`dealType=自社株買い`（提出者分類ではなく発行体自身の取得。kujira-watch側も同日追加）、`dealAmount`=上限金額（億円）、`ratioChangePct`=発行済比率、`filerName`は付けない。X個別投稿はしない（`x_buyback.py`が担う）。アイキャッチ・株価チャート・解説図（`web/article_figures.buyback_article_figures()`＝過去の決議と今回の取得上限金額の比較）は`publish_blog_articles`の共通処理を使う |
...
| `video/youtube_metrics.py` | **YouTube（Shorts）の再生数・登録者数の記録**（手動実行専用）。チャンネル(`@kujira-watch`)の公開動画の統計をYouTube Data API v3から取得し、Supabaseの`youtube_videos`（最新値）・`youtube_video_metrics`（日次スナップショット）・`youtube_channel_stats`（登録者・総再生・本数）に保存する（`supabase/create_youtube_metrics.sql`）。**動画パイプラインは毎営業日アップロードしているのに成果を1つも記録していなかった**（動画IDすら保存しておらず、何本出して何回見られたかを追えなかった＝2026-08-27に発覚）。実測は総再生4,747回・登録者3人・公開7本で、X（60日で34インプレッション・フォロワー0人）より桁違いに届いている一方、GA4上のサイト流入は28日で22セッション＝再生の0.46%だった。認証は**サービスアカウント**（`gcp_key.json`）。公開動画の統計はこれで読めるため、アップロード用のOAuthリフレッシュトークン（scopeが`youtube.upload`だけで統計を読めない）を取り直す必要はない。動画IDは投稿側に記録処理を足さずチャンネルのuploadsプレイリストから辿るので、過去分も後追いで集計できる。`--report`で尺別の平均再生（実測: 60秒以下3本が平均1,224回、60秒超4本が平均571回）と1本ごとの内訳を表示。1本も取れないときは終了コード1を返す |
| `video/youtube_auth.py` | YouTubeのリフレッシュトークンを取得する**ローカル1回きり**のスクリプト（CIでは使わない）。loopback(http://localhost:8765)で認証コードを受け取る |
| `tools/daily_log_review.py` | **日次ログレビュー（AIフィードバック）**（daily_log_review.yml、平日0:30 JST）。GitHub REST API（`gh api .../actions/runs`）で直近24hの全ワークフロー（自分自身を除く。ci.yml は失敗runのみ。ファイル`path`で束ねるので ops.yml の Keepalive/Watchdog のようにジョブ名で表示される run も漏れない）の実行結果とログを取得し、エラー・警告・記事タイトル・投稿/見送り行と各ステップの先頭・末尾だけに圧縮（**ステップの切り出しはログ本文の`##[group]Run <コマンド>`で行う**。`gh run view --log`はステップ名を解決できないと全行を`UNKNOWN STEP`にするため（gh 2.92でdaily_alertの996行すべてが該当）、これが無いと1 runが丸ごと1ステップに潰れ、先頭15行＋末尾40行＋重要行しか残らず中盤のステップが必ず落ちていた。予算超過時は全文保持ステップを避けて通常ステップから縮める）（`condense_log`。LINE送信本文を含むマーケットタイミングのステップは全文保持。`market_timing_alert.py` はウォッチリスト未登録時のフォールバック送信でも本文を標準出力に出す＝この経路だけ本文が無く、UX観点が3日連続で「LINE本文がログに無いため判断不可」になっていた）。**圧縮の解像度はrunごとに変える**: 失敗runは全文（`_MAX_CHARS_PER_RUN`=40,000字）、成功runはそのワークフローの最新1本のみ通常圧縮（12,000字）、同じワークフローの2本目以降の成功runは`signal_only=True`で重要行だけ（2,000字、該当が無ければ「静かに成功」の1行）に落とす（EDINET Blog Hourlyが平日13本走るため、入力の8割が同一ログの繰り返しになっていた＝2026-08-24の実測で248,678字中198,042字。この改修で入力は81,798字＝67%減）。さらにSupabaseから成果物スナップショット（X投稿文と反応・フォロワー推移・`blog_crawler_log`の人間推定PV＝`bot_name='Browser'`かつ1IP≤100PV/8日・上位ページ・当日ランキングの推奨ラベル内訳）を付けてClaude（claude-opus-5）に渡す。**週次の`perf_check.yml`は24時間の対象期間に入らない日でも最新run1本を「※24時間の対象期間外・N日前」と注記して添付する**（`_REFERENCE_WORKFLOWS`。月曜のみ実行のためフロントエンド観点が毎日「ログから判断不可」になっていた）。さらに**前回レビューの提案**（`daily-review` Issueの最新コメントから`extract_prev_proposals()`が「今週やる3件」「改善提案」の2節だけを抜き出す。全文を渡すと入力が倍増し当日ログの読解が前回の記述に引きずられるため。上限6,000字、取得失敗時は付けずに続行）を添える。SRE／クオンツ運用／プロダクト／SEO・GEO／UX・デザイン／バックエンド／フロントエンド／PdM の8観点で「健全性サマリー／**前回提案の消化状況**（提案ごとに ✅消化/❌未消化/❓判定不可 と根拠1行。未消化を改善提案に再掲するときは先頭に`[再掲]`、消化済みは載せない）／異常・注意点／SEO・GEO所見／UX・デザイン所見／エンジニアリング所見／PdM所見（KPIと今週やる3件・やらない事）／改善提案（観点名付き）／LINE要約」をMarkdownで書かせ、全文は`daily-review`ラベルのIssue1本にコメント追記＋Actionsジョブサマリー、「LINE要約」節だけをLINEへpush。`--dry-run`で標準出力のみ、`--since-hours N`で対象期間変更。`output_config.effort`は`medium`（日次のログ確認にhighは過剰。深掘りしたい日はworkflow_dispatchで個別に上げる）。1回あたり入力約8万字・API費用は概ね$0.3前後（入力削減とeffort調整の前は入力25万字・$1前後） |
| `tools/output_heartbeat.py` | **当日の成果物ハートビート**（ops.yml heartbeat、平日22:00 JST）。ワークフローの成否ではなく成果物そのもの（microCMSの当日公開記事数・Supabase `x_posts` の当日投稿数と`kind='video'`のクロス投稿・素材側は**EDINET API を直接叩いた当日の大量保有件数**（`count_edinet_disclosures()`、`EDINET_API_KEY`必須）と`tdnet_buybacks`）を数え、「EDINETに開示があるのにDBが0件（保存の故障）」「素材があるのに記事0件」「X投稿0件」「記事はあるのに動画0本」をLINEへ通知する。素材をSupabaseの`edinet_large_holdings`から数えていた頃は、保存側が壊れると素材も成果物も同時に0になり「開示が無い静かな日」と区別できず無言で通していた（2026-08-26〜27）。EDINETを引けなかった場合(-1)のみDBの件数にフォールバックする。各ワークフローが`continue-on-error`で緑のまま止まる無言停止を検知するための見張りで、Claudeを一切使わない。開示が無い日（祝日等）の0件は異常としない。取得できなかった件数(-1)では判定しない。`--always`で正常時も1通、`--dry-run`で送信せず本文表示、`--strict`で異常時に終了コード1 |
| `tests/test_daily_log_review.py` | 日次ログレビューのユニットテスト（ログ圧縮の残す行/落とす行・全文保持ステップ・サイズ上限・run絞り込み・path別グルーピング(CIは失敗のみ)・成果物スナップショットの人間PV判定と整形・LINE要約抽出・メッセージ組み立て・`signal_only`圧縮（重要行だけ残す/該当なしは空/全文保持ステップも圧縮する/通常圧縮より十分小さい）・run種別ごとの文字数上限の大小関係・前回レビューからの提案抽出（対象2節のみ持ち込む/該当節が無ければ空/上限で切り詰め/システムプロンプトが消化状況の見出しと`[再掲]`を要求している）・`UNKNOWN STEP`ログの`##[group]Run`による切り直し（ステップ数/中盤のLINE本文が残る/予算が厳しくても全文保持ステップは残る/BOM除去）・対象期間外runの経過日数注記、27件）|
...
| `tests/test_cleanup_duplicate_blog_articles.py` | ブログ重複記事クリーンアップの削除対象選定ロジック（先発を残し後発を削除・別提出者/別日は対象外・同一提出者でも比率変化幅が違えば対象外・filerName空の旧記事は対象外・自社株買い記事はtagsで判別し銘柄×開示日で重複判定）のユニットテスト（6件）|
| `tests/test_rewrite_thin_blog_articles.py` | 既存ブログ記事の本文リライトツールのユニットテスト（HTMLタグ・株価チャート`<figure>`を除いた可視文字数算出・PATCH用ペイロードが本文のみを含むこと・リライト後も株価チャートは末尾／解説図は本文中に戻すこと）のユニットテスト（6件）|
...
| `tests/test_publish_blog_articles.py` | ブログ記事自動投稿の判定ロジック（金額概算・発行済株式数取得のリトライ/impliedSharesOutstandingフォールバック・記事生成JSONパース・投資家分類マスター参照とClaudeフォールバック/保存・売り方向のtagsタグ付け/プロンプト分岐・重複防止・権限エラー時の早期打ち切り・投稿/更新のセレクト配列形式への自動リトライ・非文字列フィールド(eyecatch等)の型不一致時は除外して再送信・PIT文脈(株価)のプロンプト反映と下落モデル水準を渡さないこと・保有比率変化幅(ポイント)/新規保有のプロンプト反映・事業内容の取得/キャッシュ/web_searchツール付与と検索結果ブロック混在時のJSON抽出/生の改行を含むJSONの許容・投資家プロフィールの取得/キャッシュ/空欄時の非創作・「※推測:」ラベル付き推測文の要求・英訳(bodyEn/ローマ字名)のプロンプト要求・決定的テンプレによるタイトル組み立て（買い/売り/新規保有・60字超過時の提出者名短縮・英語名フォールバック）・冒頭アンサー文のプロンプト指示・ratioChangePct（売りは負値）のpayload付与・filerName＋ratioChangePct照合による重複判定（旧記事はdealAmountフォールバック）・PATCH更新への型不一致自動リトライ・アイキャッチ画像生成/アップロード・株価チャート画像生成/アップロード/本文埋め込み・ホームページ「注目」枠と同じdealAmount降順での注目記事id抽出・直前保有割合を優先した変化幅算出／全売却の扱い・株価キャッシュ欠損時のyfinanceフォールバック・比率不変時のスキップ・大幅訂正記事の投稿(dealAmount=0/tags訂正/タイトル)・単一開示日の重複判定緩和・直前保有割合が未取得の変更報告書の持ち越し・待っても直前保有割合が入らない変更報告書の変化幅None／記事化スキップ／新規保有と判定しないこと・AI常套句検出時の再生成と字数優先の採用・事業内容/プロフィールのネガティブキャッシュ（試行済みはClaudeを呼ばない・RECHECK_DAYS経過後は再挑戦・空文字でもchecked_atを刻む・空文字でdescription/profileを上書きしない・`max_uses`が2であること・タイムゾーン無しや不正な値の扱い）・利用上限の検知（後続呼び出しを止める・上限エラーはネガティブキャッシュに含めない・529等の一時的失敗では止めない））のユニットテスト・アイキャッチ写真の記事ごとの選び分け（seedで候補80枚から決定的に選択・同一seedは同じ写真・クエリ単位の候補キャッシュで検索APIを1回に抑える）・数値トークンを割らない折り返し・保有比率0%の銘柄行（売り記事は「全株売却」・自社株買いの欠損値は数字を出さない）（117件、ネットワークは全てモック）|
| `tests/test_writing_style.py` | AI常套句・単調文末の検出（`lib/writing_style.py`）のユニットテスト（常套句の検出・自然な記事文の非検出・同一文末4連続の検出・文末が変化する文の非検出・HTMLタグ除去・空入力・プロンプト用ルール文の波括弧非含有、7件）|
...
- **`max_uses` は費用に直結する**: 増やすときは検索料と入力トークンの両方が増えることを踏まえて判断する。
