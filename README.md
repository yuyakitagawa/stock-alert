...
| `tools/reclassify_blog_articles.py` | **既存ブログ記事の投資家分類（dealType）一括再分類**（手動実行専用）。旧dealType体系（インサイダー買い/日系ファンド買い等）で公開済みの記事を、`classify_filer()`が返す新14分類へ移行する。各記事のstockCode+dealDateから`edinet_large_holdings`を逆引きしてfiler_nameを特定（同一銘柄・同一開示日に複数提出者がいて一意特定できない記事はスキップし一覧表示）、`update_article()`でmicroCMSをPATCH。`--dry-run`で変更内容の確認のみ可 |
