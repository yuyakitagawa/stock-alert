...
- 🥈 A買い: ネット ≥ 6%（標準推奨、フィルター下限と一致）
- 🔴即切: 20日リターン < −10%（損切りライン割れ）

バックテスト（10期間: 2024/04〜2025/10、4000銘柄）: 平均+8.5% / 日経アルファ+6.4%
...
- **新規候補 Top5**：スクリーナー通過 + モデル予測ネットスコア ≥6%（コンフリクト除外済み）の銘柄
...
| 保有日数 | 売り閾値 | 意図 |
|---|---|---|
| 0〜30日 | net < -5% | 通常フェーズ：強いネガティブシグナルのみ売り |
| 31〜63日 | net < 0% | 中期フェーズ：ネットがマイナスになったら売り |
| 64日〜 | net < +6% | 後期フェーズ：モデルホライズン外。プラス圏でも弱ければ売り |
