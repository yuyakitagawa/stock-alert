...
| `tools/backfill_article_figures.py` | **既存ブログ記事への解説図バックフィル**（手動実行専用）。本文は作り直さず（＝Anthropic API不使用）、`web/article_figures.py`の図だけをSupabaseの既存データから作ってmicroCMSへPATCHする。株価チャート以外の`<figure>`が既にある記事は処理済みとして除外し、大量保有報告書の記事は`build_context_facts()`→`build_article_figures()`、自社株買いの記事は`tdnet_buybacks`の過去決議→`buyback_article_figures()`で図を作る。保有比率は`ratioChangePct`（変化幅）では復元できないため、`edinet_large_holdings`を`stockCode`+`dealDate`+`filerName`で引き、無ければタイトルの「X%」から拾う。本文の`<figure>`を一旦外して解説図を段落間に差し込み、株価チャートは末尾へ戻す（`bodyEn`にも同じ画像を英語キャプションで入れる）。記事1件あたり最大3枚のメディアが増えるため、`--dry-run`→`--limit`で少しずつ流す。`--days`で開示日の範囲を絞れる |
...
| `tests/test_backfill_article_figures.py` | 解説図バックフィルツールのユニットテスト（株価チャートだけの記事を未処理とみなす判定・解説図は段落間／株価チャートは末尾へ戻すマージ・処理済み/コード無し/期間外の除外・自社株買い記事の判別、5件）|
