...
SEOでは、記事タイトルに銘柄名・証券コード・提出者の正式名称・保有比率・開示種別を含める。
長い提出者名もデータ上では省略せず、既存記事に保存された旧省略タイトルはWeb表示時に
`filerName`から復元して、HTML・OGP・構造化データへ同じ正式タイトルを出す。
...
（ブログ記事自動生成・投稿。基準を満たす開示は全件記事にする＝日次上限なし）→ tools/enrich_buybacks.py → web/publish_buyback_articles.py
（TDnet自社株買い決定の記事化。上限10億円以上 or 発行済3%以上）→ **取りこぼしのbackfill**（専用cron `30 12 * * 1-5` の便と手動実行のみ。GitHubのscheduleは大きく遅延することがあり（実測: 2026-08-27の12:00 UTCの便が16:57に起動）、実行時の`date -u +%H`で「当日最終便か」を判定するとbackfillが永久にスキップされるため、`github.event.schedule`＝起動したcron式そのもので分岐する。手動実行の`backfill_max`入力で1便あたりの投稿上限を上書きできる（積み残しをまとめて消化する用。未指定なら`BACKFILL_MAX_ARTICLES`。シェルへの注入を避けるため環境変数で受けて整数か検証する）。手動実行の`backfill_since`入力（`YYYY-MM-DD`。`--backfill-since`へ渡す）で対象をその開示日以降だけに絞れる。古い順に消化する仕様のため、特定期間だけ埋め直したいときに日次予算を古い開示へ先に使い切らせないための入口（用途: 日次予算$0.15で記事が15〜19本に頭打ちだった2026-09-01〜09-04の埋め直し）。ジョブの`timeout-minutes`は120（実測で1本あたり約40秒＝15本で16分）。
...
切り替えた際、便数を3便（`0 0,6,9 * * 1-5`）へ、記事を大量保有・自社株買いそれぞれ日2本
（`DAILY_MAX_ARTICLES`）へ絞ったが、**2026-09-03に日次上限を撤去し、基準を満たす開示は全件記事にする
運用へ戻した**（オーナー指示「開示案件は記事にしてほしい」。日2本では開示の大半が記事にならず、
TOPの「N件の開示」が実態より少なく見えていた）。**2026-09-06に足切りを「5億円以上 or 1.5pt以上」から
「10億円以上」のみへ変更**（オーナー指示。実測08/01〜09/05の24開示日・対象1,161件で706件＝日29.4本 →
429件＝日17.9本）。記事にしなかった開示は`/date/[date]`の「この日の対象開示」に全件並べる。便も平日9〜19時JSTの
毎時（`0 0-10 * * 1-5`、11便）へ戻した。20〜21時JSTの便はX日次サマリー専用だったので復活させない。
記事1本ごとのX投稿の呼び出しは2026-08-30に外したまま（Xは土曜の週次まとめ1本だけ残す。後述のx_post.yml）。edinet_large_holdings はSupabase経由で
...
日次予算（`lib/api_budget.py`、既定$1.2/日）に達して残りの候補を打ち切った便は設計どおりの停止なので
赤くしない（台帳の「日次予算で打ち切り」＝正常な見送り。残りは翌日UTC 0時以降の便が拾う）。
Anthropicの月次利用上限で打ち切った便は復旧まで記事が出ないので「API利用上限で打ち切り」＝異常のまま
赤くする。どちらも上限に達した時点で残りの候補の株価・DB照会ごと打ち切る（2026-09-02。それまでは
backfill便が14本公開した後の残り75件を1件ずつ「記事生成に失敗」に数え、3日連続で赤くなっていた）。
...
その他ワークフロー: seo_loop.yml（毎日08:47 JST。①`tools/indexnow_ping.py`で直近2日の更新URLをIndexNowへ通知 ②`tools/seo_loop.py`でSearch Consoleを6フラグで読み、GA4の着地後CTAと突き合わせてジョブサマリーとLINEに出す。②はSecret `GCP_SERVICE_ACCOUNT_JSON`（任意で`GA4_PROPERTY_ID` / `GSC_SITE_URL`）が未登録ならスキップ）、ci.yml（main へのpush時にテスト全件を実行。サムネイル・画像生成のテストが日本語フォントを要求するため、edinet_blog.yml / video_post.yml と同じ `fonts-noto-cjk` を入れてから走らせる。`tests/test_ga4_clicks.py` が `google.oauth2` をmock.patchするため `google-auth` も入れる＝未導入だと `module 'google' has no attribute 'oauth2'` で1件だけ落ちる）、
...
### Supabase Free枠の制約（DB 500MB / egress 5GB per month）

2026-09-07に **egressが5GB/月を超えて全RESTが402**（`exceed_egress_quota`）で止まり、Webエクスポート・
EDINET Blog Hourly・サイトのISR再検証が全て失敗した（Next.jsのキャッシュが古いデータを200で返し続けるため
「落ちて見えない」）。同時にDBも544MBで500MBを超えていた。以下で 337MB / 推定0.5GB per month まで落としてある。

| 効かせている手 | 実装 |
|---|---|
| 全銘柄の株価をRESTで引かない（月1.1GB→0.005GB） | `lib/price_store.py` のローカルミラー |
| 銘柄コード一覧を全行SELECTしない（1回27MB→60KB） | RPC `price_cache_codes()` / `price_cache_coverage()` |
| ランキング日付を全行SELECTしない（1回12MB→数KB） | RPC `gen_ranking_dates()` |
| クローラーログのUAを行ごとに持たない（62MB→30KB） | `blog_crawler_ua` へ正規化するトリガー |
| 被リンク調査系botを入れない（月4.3万行＋その再検証） | `robots.txt` のDisallow＋`crawlers.ts`の`IGNORED_BOTS` |
| 保持期間を切る（月160MBの増加を止める） | `tools/purge_supabase.py`（ops.ymlのheartbeat便） |
| 銘柄ページの再検証を7日に1回にする（月約600MB） | `src/app/(ja)/stocks/[code]/page.tsx` の`revalidate` |

制限中も **Management APIの`execute_sql`は通る**ので、障害調査はREST経由ではなくSQLで行う。
ページングするSELECTで全件揃っている前提の呼び出しは `sb.select(..., strict=True)` を使うこと
（既定は途中のページが失敗しても取れた分だけ返すため、穴の空いたデータで先へ進む）。
...
| `tools/fetch_earnings_forecast.py` + `lib/earnings_forecast.py` | **決算短信の会社予想取得**（daily_alert.yml Step 2d2、continue-on-error）。TDnet（やのしん）の決算短信サマリーXBRL（インラインXBRL）から会社予想の純利益・営業利益・売上を取り出し、`jquants_fin_summary` に `doc_type='短信予想'` の行として保存する。**J-Quants Free が2026-04-24分で終了し、以降の決算はEDINET（有報・半期報告書）由来で会社予想が載らない**ため、`fnp` が2026-04-25以降0件になり、押し目買い通知の「予想増益」が判定できない銘柄が増えていた。予想値は実績と同じ要素名で contextRef に `ForecastMember` が入り、期は接頭辞（`CurrentYearDuration`=今期／`NextYearDuration`=来期）で分かる。通期短信は来期予想を優先し、連結が無ければ単体、レンジ予想の上限/下限は使わない。**TDnetはXBRLを約1か月で消す**ので遡れるのは直近31日ぶんだけ（それ以前は404）。`--days`/`--from`/`--to`/`--dry-run` |
| `tools/fetch_company_profiles.py` + `lib/company_profile.py` | **有報の事業情報取得**（daily_alert.yml Step 2d3、continue-on-error）。EDINETの有価証券報告書（docTypeCode=120）のXBRLから「事業の内容」「対処すべき課題」「従業員の状況（人数・平均年齢・平均年収・平均勤続年数）」を抜き、Supabaseの `company_profile`（`supabase/create_company_profile.sql`）へ保存する。押し目買い通知に「どんな会社か」を1行添えるのが用途。**Claude APIは使わない**（要約せず、本文から1文を機械的に選ぶ）。`headline()` は「当社グループは…により構成されております」のような構成説明と「なお〜」の補足を飛ばし、事業・製造・販売などの語を含む文を優先して40字で切る。本文は先頭800字だけ保存する（有報1本のXBRLは3MB前後あり、Free枠500MBに収めるため）。保存済みの銘柄はスキップするので日次は新着のみ。`--days`/`--from`/`--to`/`--force`/`--dry-run` |
| `web/dip_buy_alert.py` | **押し目買い候補のLINE通知**（daily_alert.yml Step 5、continue-on-error）。日経平均の当日騰落が-2%以下の日だけ動き、当日-4%以下に下げた銘柄のうち株価300円以上・売買代金1億円/日以上・PBR<1.5（株式分割を補正）・会社予想の純利益が前期実績より多い・従業員1000人以上（`company_employees`、`lib/employees.py`経由。行が無い銘柄だけYahooで補って書き戻す）を、β（過去250日）の高い順に**1銘柄2〜3行（銘柄名＋コード／騰落・β・PBR・増益率）**でオーナーのLINEへ送る（`company_profile` に事業情報があれば3行目に「└ 事業の要点」を添える。件数の上限は設けず、1通の4,000文字に入る分だけ並べて残りは「…ほかN銘柄」。末尾の条件文が切れないように本文側で調整する）（`notify.push_once`で同じ日の二重送信を防ぐ）。同じ銘柄の再通知は抑えない（買うかは人間が判断する）。決算は前日までの開示だけを使い、会社予想は直近の本決算より後の年度のものに限る（2026-04-25以降はEDINET由来で予想が入らないため、判定できなかった件数を本文に出す）。本決算が400日より前の銘柄には「⚠決算古」を付ける。当日の終値がある銘柄が1,000未満なら株価更新の失敗とみなしてLINEで警告する。`yahoo_price_cache`は過去行を分割調整しないので、分割日の騰落はβから除き、当日が分割日の銘柄は候補にしない。`--dry-run`で送らずに本文を表示、`--date`で過去日を判定。条件の根拠は`docs/dip_buy_strategy.md` |
| `tools/fetch_employees.py` + `lib/employees.py` | **従業員数（Supabase `company_employees`、`supabase/create_company_employees.sql`）の更新**（daily_alert.yml Step 2h、continue-on-error）。Yahoo Financeの`fullTimeEmployees`（連結・現在値）を、未取得の銘柄→取得日が古い順に`--limit 400`銘柄ずつ取る（約3,800銘柄を10日弱で一巡）。取得から30日以内（`--max-age-days`）は取り直さない（従業員数は年1回しか変わらない）。Yahooに値が無い銘柄もNULLで保存して毎日取り直さない。ただし**取得エラー（レート制限・通信断）はNULLとして保存しない**（`fetch_one()`が「取れたか」と値を分けて返す。混ぜるとレート制限中の銘柄が「値が無い」扱いになり30日間取り直されない＝2026-09-19の初回投入で2,432件が誤ってNULLになり、通知の候補23件中16件が漏れた）。連続15件断られたら`RateLimited`で打ち切り、残りは次回。通知のたびに候補数ぶんYahooへ問い合わせると急落日に数分かかり、並列取得の一部がレート制限で空振りして候補から漏れるため、テーブルに持つ。`--limit 0`で全銘柄（初回の一括投入） |
...
| `lib/api_usage.py` | **Claude API利用量の記録**。`record(resp, task=...)` が`messages.create()`のレスポンスからトークン数・キャッシュ・`server_tool_use.web_search_requests`を拾い、(UTC日付, ジョブ, タスク, モデル)単位でプロセス内に集計して、プロセス終了時(atexit)に`api_usage`テーブルへ追記する（`flush()`）。1呼び出しごとにHTTPを足さないため既存の処理時間に影響しない。コストは公開単価表（Haiku 4.5=入力$1.00/出力$5.00 per 1Mトークン、キャッシュ書込×1.25・読出×0.1、web_search $10/1,000検索）からの**推定値**でありAnthropicの請求額そのものではない。記録は計測であって目的ではないため、集計も書き込みも失敗を呼び出し側へ伝播させない（usageを持たないテストのMockを渡しても落ちず行も作らない）。書き込みのついでに当月(UTC)の累計を月次上限（`DEFAULT_MONTHLY_BUDGET_USD`=**$15**、環境変数`ANTHROPIC_MONTHLY_BUDGET_USD`で上書き、0で監視オフ）と突き合わせ、**50%/80%/100%を超えたらLINEへ通知**する（`check_budget()`）。通知は(月, 水準)単位で`notify.once()`が重複排除するため、毎時ジョブが同じ警告を何十通も送ることはない。さらに**日次予算**（`DEFAULT_DAILY_BUDGET_USD`=**$1.2**、環境変数`ANTHROPIC_DAILY_BUDGET_USD`で上書き、0で無効）を持ち、当日(UTC)の記録済み＋未送信の合計がこれを超えたら`flush()`してから`api_budget.stop_for_daily_cap()`で**その日の以降のClaude呼び出しを打ち切る**（`check_daily_cap()`、`record()`の中から毎回呼ぶ＝呼び出し側の付け忘れを無くすため）。当日の記録済みぶんはプロセス内で1回だけ読む（毎回読むと記事26本の便でSupabaseへ26往復する）。月次上限に当たると復旧まで1ヶ月止まるのに対し、日次なら被害が翌日UTC 0時までに収まる。上限は2026-08-30に記事を日2本へ絞った際$1.2から$0.15へ下げ、2026-09-03の日次上限撤去（基準を満たす開示を全件記事化＝約26本/日）で$1.2へ戻した（記事本文1本の実測$0.0092×約26本≒$0.24＋未取得の会社説明1社約$0.05の分が定常）。`lib/api_budget.py`は上限に**到達してから**止めるだけなので、その手前で気づくために置く。上限到達時に「どの用途がいくら使ったか」を示す記録が無く、バックフィルのログをgrepして犯人を推定するしかなかった（2026-08-23）ため2026-08-29に導入 |
...
| `tools/purge_supabase.py` | **Supabaseの保持期間の適用**（ops.ymlのheartbeat便＝平日1回）。DB側の関数`purge_blog_crawler_log()`（Browser 30日 / その他bot 60日 / **AIクローラー400日**＝GEO実験の観測データなので長く残す / 未参照UA）、`purge_gen_rankings()`（150日＝サイトの株価チャートが使う90営業日を確実に賄う長さ）、`purge_yahoo_price_cache()`（620日＝`rf_train_v3.HISTORY_DAYS`=600に余裕を持たせた長さ）を順に呼ぶだけ。放置すると月160MB増え、実際に2026-09-07に544MBまで育ってFree枠(DB 500MB)を超えた |
| `tools/traffic_report.py` | **アクセスログの機械/人の切り分け**（手動実行専用）。`blog_crawler_log`の"Browser"判定（既知botのUAでなくブラウザUAのもの）には機械が大量に混ざるため、4段階で除外して残りを表示する（UAは`blog_crawler_ua`へ正規化されているので`ua_id`から引き当てる）: ①**self**=自サイトからの自己リクエスト（`::1`/`127.0.0.1`）②**bot_ua**=UAに`bot`/`crawler`/`spider`/`externalagent`/`+URL`を含む自己申告（`crawlers.ts`の`BOT_PATTERNS`に載っていない新顔をここで拾い、名前を集計して表示するので登録漏れに気づける）③**heavy_ip**=1IPあたりPVが閾値（`--max-pv-per-ip`、既定100）超 ④**cookieless_ua**=同一UAで`--min-ua-pv`（既定20）以上のPVがあるのに`visitor_id`がPVとほぼ1:1（比0.9以上）＝クッキーを保持しない機械。残りのPV・IP数・2PV以上見た訪問者数・JST時間帯分布・記事ページ率・人気pathを表示する。**実測2026-08-28（1日）**: Browser 3,573PVの内訳がself 187 / bot_ua 1,384（meta-externalagent 1,031・Amazonbot 337）/ heavy_ip 1,421 / cookieless_ua 525で、残った人の候補は50〜100PV。同日の全リクエスト19,658件のうち81.8%はUAでbotと名乗っており、最大はGoogleOtherの13,497件。クッキー(visitor_id)の**有無**とJS実行(`/api/counter`)の有無は判別に使えないことを実データで確認済み（どちらも1人あたり74〜75PVでクローラーが同条件を満たす）ため、同じ検証を繰り返さないようdocstringに記録している（cookieless_uaは「有無」ではなく「PVに対する発行数の比」で見るので別物） |
...
| `tools/seo_loop.py` | **毎朝のSEOループ（Read段）**（`seo_loop.yml`で毎日08:47 JST、手動実行も可）。`gsc_report.py`は28日の全体像で毎朝読むには多すぎるため、直近7日のSearch Consoleを6フラグだけで読む: ①almost_there（3〜20位・非ブランド）②no_clicks（表示500回超かつCTR0.5%未満）③decaying（ページのクリックが前週比-30%以上、前週5クリック未満は除外）④untargeted（10位以内なのにページ`<title>`にクエリ語が無い。title取得は上位30ページまで、取れないページは判定しない）⑤wrong_intent（比較/計算/定義クエリとページ種別の不一致）⑥ai_mode（7語以上、日本語は20字以上）。さらにGA4の着地ページ別CTAクリック（着地セッション内の`click`イベント）とパスで突き合わせ、**leak**（GSC週10クリック以上でCTA0）と**converts**（GSCクリックあたりCTAが多い）を出す。GA4未設定なら突き合わせ節だけ省略。フラグ以外は出さない。`--limit` / `--no-titles` / `--out` / `--line`（フラグごとの件数と上位3件だけをLINEに送る。該当なしの節は載せない）。Anthropic APIは使わない |
| `tools/indexnow_ping.py` | **IndexNowへの更新通知**（`seo_loop.yml`で毎日）。ChatGPT検索はBingの索引を使うため、サイトマップ（`/sitemap.xml`→子サイトマップ）の`<lastmod>`が直近2日のURLをIndexNow（Bing・Yandex等で共有）へ送る。lastmodの無いURLは送らない。鍵は公開前提の値で`kujira-watch/public/<鍵>.txt`に置いて所有確認させる（環境変数`INDEXNOW_KEY`で差し替え可）。`--days` / `--dry-run` |
| `tools/geo_report.py` | **GEO（生成AI検索での引用最適化）のPDCAレポート**（手動実行専用）。「AIに読まれているか」と「AIから人が来ているか」を1画面で前期間と比較する。AIクローラーはJSを実行しないためGA4に一切載らず、巡回の実態は`blog_crawler_log`にしか無い。逆にAIの回答からリンクを踏んだ人の訪問はGA4（`sessionMedium=ai-assistant`）にしか無いので、両方を1つのレポートに並べる。出すもの: ①AIクローラー別の巡回数 ②ページ種別の内訳 ③`ChatGPT-User`/`PerplexityBot`が取りに来たページTOP（**回答生成のためのその場取得＝引用の代理指標**。学習・インデックス目的の`GPTBot`/`OAI-SearchBot`とは意味が違うので分ける）④AI経由のセッション・エンゲージメント率・着地ページ（GA4）⑤AIクローラーが当たっている**存在しないURL**とリダイレクト経由の旧URL。⑤はルートの形（`EXACT_PATHS`/`PREFIX_PATTERNS`）と突き合わせる実装で、ページを増減したらここも直す。**実測2026-08-29（14日）**: OAI-SearchBot 6,087回・PerplexityBot 1,777回・ChatGPT-User 1,141回に対し、AI経由の実訪問はGA4で6セッション（全体の1.4%）。廃止済みの`/disclosures`に58回・存在しない`/articles`に18回当たっていたのをこれで発見した。`--days`で期間、`--limit`で表示行数。GA4の鍵・プロパティIDが無い場合は④だけスキップして残りを表示する 2026-09-04から`blog_crawler_log.host`を読み、AIクローラー別の巡回数の下に英語版（`en.kujira-watch.com`）ぶんの行を出す。存在しないURLの判定もホスト別（英語版は`tools/en_crawl_report.py`の`en_path_status()`を使う） |
| `tools/en_crawl_report.py` | **英語版サブドメイン（`en.kujira-watch.com`）のクローラー巡回レポート**（手動実行専用）。英語版は2026-08-29に`/en`配下を廃止した後、「ディレクトリではなくサブドメインなら別サイトとして巡回されるか」を実測するため2026-09-04にmicroCMSに残っている英訳済み記事だけで再開した（新しい英訳は作らない）。`blog_crawler_log`の`host`列（同日追加。NULLは日本語版）で日英を分け、①英語版のクローラー別ヒット数（Browserも1行＝人の目安）②同じクローラーの日本語版ヒット数と英語版の比率③英語版の日別ヒット数（来始めた日が分かる）④英語版で踏まれているパスTOP⑤英語版に存在しないURLへのアクセス（`/stocks/...`等、日本語版にしか無いページへのリンク切れ・rewrite漏れの発見用）を前期間比つきで出す。`--days`（既定14）`--limit` |
...
| `tests/test_edinet_financials.py` | EDINET決算XBRL抽出（`lib/edinet_financials.py`）のユニットテスト（IFRS会社で単体より連結を優先・親会社株主帰属利益・連結なし会社の単体フォールバック・タグ完全一致・取得対象は有報/半期報のみ・保存済み書類の読み飛ばし・日ごとの保存・`--force`での取り直し）（10件）|
...
| `tests/test_earnings_forecast.py` | 決算短信XBRLからの会社予想抽出のユニットテスト（今期予想の取り出しとscale解決・通期短信で来期予想を優先しNextYearInstantが無ければ今期末の1年後を使うこと・連結優先・赤字予想のsign解決とレンジ予想の除外・予想が無い短信/壊れたzipでNoneを返すこと、5件）|
| `tests/test_company_profile.py` | 有報からの事業情報抽出のユニットテスト（テキストブロックのHTML除去と見出し落とし・人数/平均年齢/平均年収の数値抽出・構成説明と「なお〜」を飛ばして事業の文を採ること・40字での切り詰め・銘柄コードや事業の内容が無い書類を保存しないこと、5件）|
| `tests/test_dip_buy_alert.py` | 押し目買い候補通知のユニットテスト（β・日次騰落・開示後の株式分割の検出と分割比から外れた急落の無視・前日までの開示だけを使うこと・直近の本決算と同じ年度の予想で増益判定しないこと・分割補正後のPBR・前期赤字/横ばいの除外・本文の2行組みと「⚠決算古」・候補300件でも1通に収め条件文を切らさないこと・予想未取得の件数表示・従業員数テーブルの取り直し順（未取得→古い順、30日以内は除外）・テーブルに無い銘柄だけYahooで補うこと・取得エラーを「値が無い」として保存しないこと/連続エラーで打ち切ること、15件）|
...
| `tests/test_publish_blog_articles.py` | ブログ記事自動投稿の判定ロジック（金額概算・発行済株式数取得のリトライ/impliedSharesOutstandingフォールバック・記事生成JSONパース・投資家分類マスター参照とClaudeフォールバック/保存・売り方向のtagsタグ付け/プロンプト分岐・重複防止・権限エラー時の早期打ち切り・投稿/更新のセレクト配列形式への自動リトライ・非文字列フィールド(eyecatch等)の型不一致時は除外して再送信・メディアURLが不正と言われたとき（`'eyecatch' field invalid`）は同じURLで一度待って再送信し、直らなければ画像なしで投稿・PIT文脈(株価)のプロンプト反映と下落モデル水準を渡さないこと・保有比率変化幅(ポイント)/新規保有のプロンプト反映・事業内容の取得/キャッシュ/web_searchツール付与と検索結果ブロック混在時のJSON抽出/生の改行を含むJSONの許容・投資家プロフィールの取得/キャッシュ/空欄時の非創作・「※推測:」ラベル付き推測文の要求・英訳を求めないこと・決定的テンプレによるタイトル組み立て（買い/売り/新規保有・60字超過時の提出者名短縮）・冒頭アンサー文のプロンプト指示・ratioChangePct（売りは負値）のpayload付与・filerName＋ratioChangePct照合による重複判定（旧記事はdealAmountフォールバック）・PATCH更新への型不一致自動リトライ・アイキャッチ画像生成/アップロード・株価チャート画像生成/アップロード/本文埋め込み・ホームページ「注目」枠と同じ注目記事id抽出（最新の取引日から3開示日ぶんに絞ってdealAmount降順・絞ると件数不足なら窓を広げる）・直前保有割合を優先した変化幅算出／全売却の扱い・株価キャッシュ欠損時のyfinanceフォールバック・比率不変時のスキップ・大幅訂正記事の投稿(dealAmount=0/tags訂正/タイトル)・単一開示日の重複判定緩和・直前保有割合が未取得の変更報告書の持ち越し・待っても直前保有割合が入らない変更報告書の変化幅None／記事化スキップ／新規保有と判定しないこと・特例報告の判定（`is_special_report()`）と履歴の無い新規特例報告の変化幅None（非特例の新規届出は全量のまま／特例でも直前比率や履歴があれば差分で計算）・AI常套句検出時の再生成と字数優先の採用・事業内容/プロフィールのネガティブキャッシュ（試行済みはClaudeを呼ばない・RECHECK_DAYS経過後は再挑戦・空文字でもchecked_atを刻む・空文字でdescription/profileを上書きしない・`max_uses`が1であること・タイムゾーン無しや不正な値の扱い）・利用上限の検知（後続呼び出しを止める・上限エラーはネガティブキャッシュに含めない・529等の一時的失敗では止めない））のユニットテスト・アイキャッチ写真の記事ごとの選び分け（seedで候補80枚から決定的に選択・同一seedは同じ写真・クエリ単位の候補キャッシュで検索APIを1回に抑える）・数値トークンを割らない折り返し・保有比率0%の銘柄行（売り記事は「全株売却」・自社株買いの欠損値は数字を出さない）・既報インデックスのページング/失敗時None・backfillの事前足切り（既報と基準未満を落とす／ビュー未収載は残す）・backfillの窓拡大と古い開示優先・既報インデックスが引けないときの中止・`BACKFILL_MAX_ARTICLES`による投稿数の頭打ち・開示側の台帳（記事を作ったことがある開示はbackfill・通常運転とも作り直さない／投稿成功時に記録する）・本文HTMLに生の改行が入ったJSONのパース・個人の提出者は説明文を落とし法人は残す・台帳（`lib/publish_ledger.py`）との結線（基準未満だけの便は正常扱い・生成失敗/投稿失敗は異常として終了コード4・公開できた候補も台帳に乗り未分類が残らないこと）・引き上げ後の足切り(10億円のみ)で旧基準(5億円/1.5pt)を落とすこと・比率変化では救済しないこと・大幅訂正は金額の足切りを迂回すること・サイト側の数値（`kujira-watch/src/lib/disclosures.ts`の`ARTICLE_MIN_DEAL_AMOUNT_OKU`/`MATERIAL_CORRECTION_DELTA_PT`/`MAJORITY_HOLDING_THRESHOLD`とFAQ本文）がPython側とずれていないこと・公開済み記事のindex基準(3億円/1.0pt)は据え置きで足切り≧index基準であること・既報照会が判定不能なときの投稿見送り（HTTPエラー／例外）・既報照会が開示日まで絞り込まれること・日次上限（当日の実績を引いた残り枠で止まること・当日ぶんが上限に達していれば0件・取得できなければ上限いっぱい・`--max-articles`の明示は当日の実績を見に行かないこと）・API予算での打ち切り（日次予算に達したら残りの候補は生成を呼ばずに「日次予算で打ち切り」＝正常終了、月次上限なら「API利用上限で打ち切り」＝終了コード4））のユニットテスト・書き出しの切り口（`pick_lead_angle()`）と報告書本文の事実（`format_filing_details()`）（152件、ネットワークは全てモック）|
...
| `tests/test_api_budget.py` | Claude API利用上限フェイルファスト（`lib/api_budget.py`）のユニットテスト（実際に返ってきた400文言の検知・クレジット残高不足の検知・429/529/500/接続断を上限と誤判定しないこと・フラグのラッチと解除・日次予算の停止を月次上限と区別できること（`daily_cap_reached()`）、8件）|
...
| `tests/test_supabase_client.py` | Supabase REST APIクライアントのリトライ挙動（一時的なネットワーク失敗・5xx/Cloudflare 52x応答のバックオフ再試行、400系は再試行しないこと、最終失敗時に呼び出し元を落とさないこと）・HTMLエラーページの`<title>`要約・キー構成が違う行の分割送信（upsert/insert_ignore）・書き込み失敗の記録とLINE通知（テーブルごと1回）・**テスト実行中に本番プロジェクトへ書き込まないこと**（upsert/insert_ignore/update/deleteを握りつぶし読み取りは通す／URLを差し替えたテストの書き込みは止めない）のユニットテスト（13件）|
| `tests/test_publish_ledger.py` | 「候補>0なのに公開0」の切り分け台帳（`lib/publish_ledger.py`）のユニットテスト（正常な見送りだけなら鳴らさない・生成失敗/投稿失敗/権限エラーは異常・理由を記録しないまま脱落した候補を異常に倒すこと・max_articles打ち切りの残りを未分類にしないこと・日次予算での打ち切りは正常扱い／月次上限での打ち切りは異常・未知の理由は安全側に倒すこと・内訳1行の組み立て・同じ原因の通知を連投しないこと、11件）|
...
| `tests/test_gsc_search_guard.py` | 削除ガード（`lib/gsc_search_guard.py`）のユニットテスト（日本語・旧/en・英語サブドメインのURLから記事idを取ること、表示0回と記事以外のURLを除くこと、検索に出ている記事を残すこと、GSCが取れないとき削除を中止すること。5件）|
...
| `tests/test_seo_loop.py` | SEOループ（`tools/seo_loop.py`）のユニットテスト（ブランドクエリ除外・almost_thereの3〜20位境界と1ページ目優先・クエリごとの代表ページ選択・no_clicksの境界・decayingの小ページ除外と消えたページの検出・ai_modeの英語語数/日本語字数・wrong_intent・untargetedのtitle不明見送り・GA4着地パス突き合わせとleak/converts・LINE要約は該当のあるフラグだけ上位3件、12件）|
| `tests/test_indexnow_ping.py` | IndexNow通知（`tools/indexnow_ping.py`）のユニットテスト（sitemapindexからの子サイトマップ抽出・lastmodでの絞り込みとlastmod無しの除外・1リクエスト上限・鍵ファイルがpublicに鍵そのものの中身でコミットされていること、4件）|
...
| `tests/test_en_crawl_report.py` | 英語版クローラーレポート（`tools/en_crawl_report.py`）のユニットテスト（英語版に実在するパスの判定、`host`がNULLの行を日本語版として扱うこと、日英の分離と404候補の抽出、英語版の記録が無いときの案内） |
...
| `jquants_fin_summary` | 財務サマリ（2026-04-24まではJ-Quants、以降はEDINETの有価証券報告書・半期報告書のXBRLから抽出。テーブル名は旧J-Quants由来）。日次（daily_alert Step 2d）は直近21日の書類一覧を見て、未保存の (code, disc_date) だけXBRLを取る（ジョブが数日落ちても次の成功日に拾う）。抽出を直したら `tools/fetch_edinet_financials.py --start <日付> --force` で取り直す。1Q/3Qは四半期報告書の廃止（2024-04）でEDINETに無く、会社予想(fsales/fop/fnp)も有報・半期報には無いため、2026-04-25以降の行は空 |
...
- **1日ぶんの予算で先に止める**: `lib/api_usage.py`の`check_daily_cap()`が当日(UTC)の推定コストを
  `ANTHROPIC_DAILY_BUDGET_USD`（既定$1.2）と突き合わせ、超えたらその日の残りを打ち切ってLINEへ1通流す。
...
| `ANTHROPIC_DAILY_BUDGET_USD` | Anthropic日次予算(USD)。超えたらその日の以降のClaude呼び出しを打ち切る（`lib/api_usage.py`の`check_daily_cap()`）。未設定なら**$1.2**（`DEFAULT_DAILY_BUDGET_USD`。2026-08-30に記事を日2本へ絞った際$0.15へ下げ、2026-09-03の日次上限撤去で$1.2へ戻した）。`0`で無効 |
