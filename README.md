...
| 条件 | 値 | 意図 |
|---|---|---|
| ネットスコア ≥ | 8% | 確信度確保（売り閾値6%との2%バッファ: 6%→8% 2026-05-16）|
| 下落確率 < | 12% | 高リスク銘柄を除外 |
| コンフリクト除外 | net ≥ 10% かつ drop ≥ 5% → 除外 | 両モデルが矛盾→偽陽性リスク最大 |
| 決算まで | 7日超 | 直前の変動リスクを避ける |

**推奨ラベル**:
- 🥇 S買い: 下落確率 < 8% かつ ネット ≥ 8%（低リスク高期待値）
- 🥈 A買い: ネット ≥ 8%（標準推奨、フィルター下限と一致）
- ⏳ 方向感なし: ネット 6〜8%（様子見。新規候補には表示されない）
...
- **新規候補 Top5**：スクリーナー通過 + モデル予測ネットスコア ≥8%（コンフリクト除外済み）の銘柄
...
| 保有日数 | 売り閾値 | 意図 |
|---|---|---|
| 0〜3日 | net < -5% | 通常フェーズ：強いネガティブシグナルのみ売り |
| 4〜63日 | net < +6% | 中期フェーズ：A買い基準を下回ったら売り（乗り換えキャッシュ確保） |
