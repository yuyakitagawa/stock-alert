...
その他ワークフロー: seo_loop.yml（毎日08:47 JSTに`tools/seo_loop.py`でSearch Consoleを6フラグで読み、GA4の着地後CTAと突き合わせてジョブサマリーに出す。Secret `GCP_SERVICE_ACCOUNT_JSON`（任意で`GA4_PROPERTY_ID` / `GSC_SITE_URL`）が未登録なら何もせず成功）、ci.yml（main へのpush時にテスト全件を実行。サムネイル・画像生成のテストが日本語フォントを要求するため、edinet_blog.yml / video_post.yml と同じ `fonts-noto-cjk` を入れてから走らせる。`tests/test_ga4_clicks.py` が `google.oauth2` をmock.patchするため `google-auth` も入れる＝未導入だと `module 'google' has no attribute 'oauth2'` で1件だけ落ちる）、
...
| `tests/test_seo_loop.py` | SEOループ（`tools/seo_loop.py`）のユニットテスト（ブランドクエリ除外・almost_thereの3〜20位境界と1ページ目優先・クエリごとの代表ページ選択・no_clicksの境界・decayingの小ページ除外と消えたページの検出・ai_modeの英語語数/日本語字数・wrong_intent・untargetedのtitle不明見送り・GA4着地パス突き合わせとleak/converts、10件）|
