...
| `web/x_filer_record.py` | **提出者ごとの3ヶ月後リターン実績をXへ投稿**（x_post.yml の `filer_record`、木曜08:00 JST）。既存の`web/x_followup.py`が「開示日ごと」の答え合わせ＝EDINETを見れば誰でも同じ集計ができるのに対し、こちらは**提出者ごとの通算成績**で、開示を1年半ぶん貯めている側にしか書けない。新規の大量保有報告書（変更・訂正を除く）について開示日終値→91日後終値のリターンを取り、`MIN_EVENTS`=10件以上、かつ**全開示平均から`MIN_EDGE_PT`=5ポイント以上離れている**提出者だけを対象にする（差が無い提出者は書く内容が無く、それでも上下を断定すると誤りになる。CI実測で「平均+3.6%・全開示平均+3.6%なので明確に下」という本文が出た）。**本文には必ず全開示平均を併記する**（地合いの良い期間は全体が上がるため、単体の「+18%」は実力を意味しない）。**証券会社・銀行・信託銀行の自己名義は除外**（自己売買や担保の建玉であって銘柄選択ではないため。旧字体の「證券」も弾く＝野村證券の255件が混じっていた）。株式分割等で終値が不連続な銘柄は`SPLIT_GUARD_PCT`=40%で除外。毎週同じ提出者が出ないようISO週番号で順に回す。Anthropic APIは使わない。`--dry-run`/`--filer`/`--list` |
...
| `tests/test_x_filer_record.py` | 提出者ごとのリターン実績（`web/x_filer_record.py`）のユニットテスト（旧字体「證券」を含む自己名義提出者を除外すること・休場は許容し長期の欠落は使わないこと・株式分割で不連続な銘柄を弾くこと・両端の終値が揃わない開示を落とすこと・10件未満の提出者を出さないこと・週ごとに別の提出者を選ぶこと・本文に全開示平均を必ず入れること・負けている提出者を「上」と書かないこと）・全開示平均と差が無い提出者を落とすこと・表示上おなじ数字で上下を断定しないこと）（10件、SupabaseとX APIは呼ばない）|
