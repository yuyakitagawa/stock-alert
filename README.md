...
x_weekly_post.yml（週次「クジラ急増ランキング」のX投稿、日曜19:00 JST、web/x_weekly_trending.py）
```
...
| `web/x_weekly_trending.py` | **週次「クジラ急増ランキング」のX投稿**（x_weekly_post.yml、日曜19:00 JST・週1回）。平日の記事投稿・日次サマリーが無い週末のタイムラインを埋める枠。kujira-watch `/trending`（`src/lib/trendingStats.ts`）と同じ「直近30日 vs その前30日」の開示件数比較をPythonへ移植し、`edinet_large_holdings`から増加件数の多い銘柄（最大3件）・投資家（最大2件）を集計して投稿する。社名の「株式会社」等は表示用に除去し、Xの280単位制限（全角2単位）に収まるよう投資家→銘柄の順で行を自動削減。急増銘柄が無い週は投稿しない。`--dry-run`で本文確認のみ可 |
