...
## スクリーナー条件（v1 現行 / 2026-05-08）

| 条件 | 値 | 意図 |
|---|---|---|
| 3ヶ月モメンタム | +5% 〜 +30% | 急騰後ミーンリバージョンを除外 |
| 20日モメンタム ≥ | -3% | 直近の崩れを除外 |
...
| vsurge（直近/20日平均）≥ | 1.3 | 資金流入シグナル |
| 日経比相対強度 ≥ | +5%（通常相場） / +10%（下落相場）| 日経をアウトパフォームしている銘柄のみ |

**下落相場判定**: 日経225の20日リターン < -5% のとき相対強度閾値を引き上げ
...
| 期間 | 上位10 平均 | 日経 | アルファ |
|---|---|---|---|
| bear（2024/7-10、円キャリー崩壊） | +20.8%* | -2.5% | +23.3% |
| 1年（2025/5-2026/5） | +7.9% | - | - |
| 3ヶ月（2026/2-5） | +5.8% | - | - |

*グランディーズ+208%の外れ値含む。外れ値除外後の平均は±0%
...
- **モデルの限界**：AUC 0.663はランダム（0.50）よりわずかに良い程度。参考指標として使い、最終判断は自分で行う。
- **2段階構造が必須**：モデル単体を全銘柄に適用しても効果なし。スクリーナー→モデルの順で使うことでアルファが出る。
- **下落相場では買わない**：日経20日 < -5% のとき、メールに赤バナーで警告。新規候補セクションは非表示。
