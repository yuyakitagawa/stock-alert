...
| `tools/screen_catalyst_candidates.py` | カタリスト候補スクリーン（GARP補助）。PBR<1.0・ROE<8%・自己資本比率>50%・流動性で割安改革余地のある構造的候補を抽出し `data/catalyst_candidates.csv` に保存 |
...
| `EDINET_API_KEY` | EDINET API v2 サブスクリプションキー（大量保有スキャナー用・任意）|
