...
| `tests/test_publish_blog_articles.py` | ブログ記事自動投稿の判定ロジック（金額概算・記事生成JSONパース・投資家分類マスター参照とClaudeフォールバック/保存・売却除外・重複防止・権限エラー時の早期打ち切り・投稿/更新のセレクト配列形式への自動リトライ・PIT文脈(株価/下落リスク水準)のプロンプト反映・事業内容の取得/キャッシュ・「※推測:」ラベル付き推測文の要求・アイキャッチ画像生成/アップロード・株価チャート画像生成/アップロード/本文埋め込み）のユニットテスト（45件、ネットワークは全てモック）|
