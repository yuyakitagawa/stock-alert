...
| `lib/data_sanity.py` | 出力データの不変条件チェック（net=rise−drop整合・確率レンジ・予測多様性等）。rank_stocks/export_to_web のリリース前ゲート＋PDCA QAで使用（alert-only）|
...
| `tests/test_data_sanity.py` | データ整合性チェックのユニットテスト（11件）|
