...
| `tests/test_daily_log_review.py` | 日次ログレビューのユニットテスト（ログ圧縮の残す行/落とす行・全文保持ステップ・サイズ上限・run絞り込み・path別グルーピング(CIは失敗のみ)・成果物スナップショットの人間PV判定と整形・LINE要約抽出・メッセージ組み立て・`signal_only`圧縮（重要行だけ残す/該当なしは空/全文保持ステップも圧縮する/通常圧縮より十分小さい）・run種別ごとの文字数上限の大小関係・前回レビューからの提案抽出（対象2節のみ持ち込む/該当節が無ければ空/上限で切り詰め/システムプロンプトが消化状況の見出しと`[再掲]`を要求している）、20件）|
...
| `tests/test_ga4_clicks.py` | GA4クリックログ取得（`tools/ga4_clicks.py`）のユニットテスト（GA4応答の整形＝1ディメンションは文字列キー/複数はタプルキー、前期間0からの増加を`(新規)`と出すこと、APIエラーの手順への翻訳＝API無効/プロパティ権限なし/プロパティID誤り、eventNameを完全一致で絞ること、期間とフィルターのリクエスト組み立て、GA4_PROPERTY_ID未設定、PVからのCTR算出と前期間比、labelカスタムディメンション未登録時の手順表示、クリック0件とAPI失敗で終了コード1、13件、GA4 APIはモック）|
