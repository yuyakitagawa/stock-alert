...
| `tests/test_publish_buyback_articles.py` | 自社株買い記事の自動投稿（`web/publish_buyback_articles.py`）のユニットテスト（規模閾値・検索クエリ型タイトル・payload（dealType/filerName無し/消却タグ）・既報スキップ・記事化に到達した候補数の計上と終了コード判定、10件）|
