...
（ブログ記事自動生成・投稿。足切り（推定3億円以上 or 保有比率1.0pt以上）を通った開示は**全部**記事にする）→ tools/enrich_buybacks.py → web/publish_buyback_articles.py
...
切り替え、便数を毎時13便（`0 0-12 * * 1-5`）から3便（`0 0,6,9 * * 1-5`）へ、記事を日2本
（`DAILY_MAX_ARTICLES`）へ絞ったが、**更新頻度が落ちてメディアとして成立しないため
2026-09-06に両方とも撤回した**（毎時13便へ戻し、日次上限は削除。足切りを通った開示は全部記事にする。
あわせて日次API予算を$0.15→$1.2へ戻した＝$0.15では足切り通過ぶんを出し切る前に打ち切られ、
2026-09-01〜09-04は毎日上限に張り付いていた）。
