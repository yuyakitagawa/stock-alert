# stock-alert

東証上場株式の機械学習スクリーニング・アラートシステム。毎日自動でランキングを生成し、メールで通知する。

## システム概要

毎朝8時（JST）にGitHub Actionsで自動実行される。

    screener.py → rf_train_v3.py → rank_stocks.py → alert_email.py
       (毎日)       (土曜のみ)        (毎日)            (毎日)

---

## ファイル構成

| ファイル | 役割 |
|---|---|
| screener.py | JPX全銘柄から条件通過銘柄を抽出してCSV保存（`--mode {v1,v2,both}` で条件切替・並走可能）|
| rf_train_v3.py | XGBoostモデルを東証全銘柄×5年データで学習 |
| rank_stocks.py | スクリーナー通過銘柄に上昇/下落確率をつけてランキング |
| alert_email.py | ランキング上位をGmailでメール送信 |
| utils.py | 共通関数（get_prices, extract_features, compute_seq_features, add_cs_rank_features 等） |
| validation.py | Purged K-Fold検証（先読みバイアス定量化）、ユニットテスト付き |
| backtest.py | バックテスト（先読みバイアスなしモード実装済み、`--screener {v1,v2}` でスクリーナー条件切替可） |
| analyze_alpha.py | スクリーナーとモデルのアルファ貢献度を分離分析 |
| generate_post.py | SNS投稿用テキストを生成 |
| sheets_helper.py | Googleスプレッドシートへの書き込みユーティリティ |

---

## モデル詳細（rf_train_v3.py）

### 学習データ
- 対象：東証プライム・スタンダード全銘柄（約3,500〜4,000銘柄）
- 期間：過去5年分（約1,800日）
- サンプリング：20営業日ごと（自己相関を低減）
- 分割：2025年1月1日より前→学習用 / 以降→テスト用（ウォークフォワード）

### 特徴量（28次元）

テクニカル10次元
- ret5, ret20, ret60, ret90：5/20/60/90日リターン
- ma5_25, ma25_75：移動平均クロス（5-25日、25-75日）
- rsi：RSI（14日）
- vol20, vol60：ボラティリティ（年率換算）
- pos52：52週レンジ内の位置

トレンド反転5次元
- drawdown60：直近60日高値からの下落率
- from_hi52：52週高値からの乖離率
- down_streak：連続下落日数（0〜1に正規化）
- momentum_accel：短期モメンタム加速度（ret5 - ret20/4）
- ma_cross_dir：MA5/MA25クロスの方向変化

出来高3次元
- vr520, vr2060：出来高移動平均比（5/20日、20/60日）
- vsurge：直近出来高サージ（直近/20日平均）

日経マクロ3次元
- 日経225の5/20/60日リターン（各サンプル時点）

60日系列要約7次元（v6で60次元から圧縮）
- autocorr_lag1：日次リターンのラグ1自己相関（モメンタム持続性）
- skew：歪度（上下非対称性）
- max_ret / min_ret：最大・最小日次リターン
- pos_ratio：上昇日数割合
- trend_slope：60日線形トレンドの傾き
- recent_vs_early：後半30日 − 前半30日の平均リターン差

### 予測ラベル
- 上昇モデル：63営業日後（約3ヶ月）に+15%以上
- 下落モデル：63営業日後（約3ヶ月）に-15%以下

### モデル精度（v7 現在）
- 上昇モデル AUC：0.663（Purged CV: 0.628±0.025）
- 下落モデル AUC：0.791（Purged CV: 0.782±0.036）

### 学習タイミング
- GitHub Actionsで毎週土曜に自動再学習
- early_stopping_rounds=50 で過学習を抑制

---

## ランキングロジック（rank_stocks.py）

ネットスコア = 上昇確率(%) − 下落確率(%)

### フィルター（除外条件）
- 連続下落日数が3日超（down_streak > 0.15）
- 直近60日高値から15%超の下落（drawdown60 < -0.15）

### 推奨ラベル

| 条件 | ラベル |
|---|---|
| ネット>=10 かつ ボラ低 | 買い可能性あり |
| ネット>=5 かつ ボラ低 | 買い可能性あり |
| ネット>=5 かつ ボラ高 | 買い可能性あり（荒れ注意）|
| ネット<-10 かつ ボラ低 | 売り可能性あり |
| ネット<-5 かつ ボラ低 | 売り可能性あり |
| ネット<-5 かつ ボラ高 | 売り様子見 |
| それ以外 | 様子見 |

### 日経225比較
各銘柄の5/20/60日リターンから日経225の同期間リターンを引いた相対強度も表示。

---

## セットアップ

### 必要なSecrets（GitHub Settings → Secrets）

| Secret名 | 内容 |
|---|---|
| GMAIL_ADDRESS | 送信元Gmailアドレス |
| GMAIL_APP_PASSWORD | Gmailアプリパスワード |
| SPREADSHEET_ID | GoogleスプレッドシートID |
| GCP_KEY_JSON | Google Cloud サービスアカウントJSON |

### 依存パッケージ
requests pandas numpy scikit-learn joblib xgboost python-dotenv openpyxl gspread google-auth yfinance

### 手動実行
    python3 screener.py                    # スクリーニング（v1+v2並走、デフォルト）
    python3 screener.py --mode v1          # v1条件のみ
    python3 screener.py --mode v2          # v2条件のみ
    python3 rf_train_v3.py                 # モデル学習（40〜70分）
    python3 rank_stocks.py                 # ランキング生成
    python3 alert_email.py                 # メール送信
    python3 backtest.py                          # バックテスト（通常期、v1スクリーナー）
    python3 backtest.py bear                     # 下落相場テスト（2024年8月クラッシュ期）
    python3 backtest.py --compare                # top-N × net-min の一括グリッド比較
    python3 backtest.py --screened               # スクリーナー特化モデル（rf_model_screened.pkl）で検証
    python3 backtest.py bear --compare --screened  # bear × 比較 × 特化モデル
    python3 rf_train_v3.py --screener-only       # スクリーナー通過時点のみで学習（→ rf_model_screened.pkl）
    python3 test_screener.py                     # スクリーナーユニットテスト

---

## 設計上の注意点

- モデルの限界：AUC 0.655はランダム（0.50）よりわずかに良い程度。参考指標として使い、最終判断は自分で行う。
- フィルターの意味：下落フィルターはルールベースの除外。モデルが「買い」と言っても直近で崩れている銘柄を弾く。
- ウォークフォワード分割の意義：将来データで学習する「先読みバイアス」を防ぐため、時系列を守って学習/テストを分割している。

---

## スクリーナー条件（2026-05-06 更新）

### v1条件（現行）

| 条件 | 値 | 追加日 |
|---|---|---|
| 3ヶ月モメンタム | +5% 〜 +30% | 当初 / 上限2026-05-06 |
| 20日モメンタム ≥ | -3% | 当初 |
| 年率ボラティリティ | 20% 〜 50% | 下限2026-05-06 |
| 株価 ≥ | 300円 | 当初 |
| 上昇トレンド | slope_up=True | 当初 |
| 出来高比 vr2060 ≥ | 1.0（20日平均 ≥ 60日平均） | **2026-05-06追加** |
| 日経比相対強度 ≥ | +5%（日経225を5%以上アウトパフォーム） | **2026-05-06追加 / 2026-05-07強化** |

※ R²（線形安定度）は2026-05-06に削除。MLモデルと重複し「過去が安定」を見るだけで将来予測に寄与しないため。

### 条件の根拠

- **モメンタム上限+30%**: 急騰後のミーンリバージョン銘柄を除外
- **ボラ下限20%**: +15%×3ヶ月の目標達成には一定のボラが必要
- **出来高比 ≥ 1.0**: 上昇に出来高が伴っている銘柄のみ通過。出来高を伴わない上昇は持続しにくい
- **日経比相対強度 ≥ +5%**: 日経を5%以上アウトパフォームしている銘柄のみ通過。0%（同等）では弱すぎるため強化（2026-05-07）

### バックテスト結果（2026-05-06）

| | 旧v1 | 新v1（現行） |
|---|---|---|
| 通常期 日経アルファ（2026/2→5、日経+8.76%） | -3.79% | -5.55% |
| 下落相場 日経アルファ（2024/7→10、日経-2.47%） | -2.18% | **+16.59%** |
| 全銘柄通過数 | 452銘柄 | 61銘柄 |

通常期は若干悪化するが、下落相場での暴落耐性が大幅改善。詳細: `screener_ab_test_log.md`

### v2条件（実験中・本番未採用）

6ヶ月クロスセクション上位30% × 3ヶ月+5%〜+30% × 52週高値-20%以内 × ボラ20%〜50%。
下落相場（2024年8月クラッシュ期）で通過ゼロとなり本番採用を見送り。条件緩和後に再検証予定。

---

## バックテスト・Alpha分析結果（2026-05-04時点）

### バイアスなしバックテスト（2026-02-03 → 2026-05-04）

- TSE全銘柄からランダム200社を対象にモデルスコアで上位30を選出
- 上位30の平均リターン **+0.62%** vs 日経225 **+8.76%** → **-8.14% 劣後**
- モデル単体を全銘柄に適用しても機能しない

### スクリーナー内Alpha分析（analyze_alpha.py）

スクリーナー通過555銘柄を2026-02-03時点のモデルスコアで4グループに分類：

| グループ | 平均リターン | vs 日経 |
|---|---|---|
| モデル上位10 | +12.57% | **+3.81%** |
| 中位10 | +10.16% | +1.40% |
| ランダム10 | +8.91% | +0.15% |
| モデル下位10 | +5.52% | -3.24% |

- スクリーナーアルファ: **+0.15%**（スクリーナー単体はほぼ日経並み）
- モデル追加価値: **+3.66%**（スクリーナー通過銘柄内でのモデルの選別力）

### 下落相場テスト（2024-07-01 → 2024-10-01）

- テスト期間: 2024-07-01 → 2024-10-01（日経 **-2.47%** の下落相場・円キャリー崩壊期）
- モデル上位30の平均リターン: **-0.81%**（日経比 **+1.66%** アウトパフォーム）
- 下落相場でも損失を軽減する効果がある
- ただしスクリーナーなしのTSE全銘柄対象のため参考値

### 下落相場でのスクリーナー内Alpha分析（2024年7月→10月）

| グループ | 実績リターン | vs 日経 |
|---------|------------|--------|
| 日経225 | -2.47% | ― |
| スクリーナーランダム10 | +1.18% | +3.65% |
| モデル上位10 | -5.47% | -3.00% |
| モデル下位10 | -11.84% | -9.37% |

スクリーナーアルファ: **+3.65%**（下落相場でも有効）
モデル追加価値: **-6.65%**（下落相場では逆効果）

結論: モデルは上昇局面向けに学習されており、下落相場ではモメンタム株を選びすぎる。
対策: 日経トレンドフィルター（日経20日 < -5%で警告）を実装済み（2026-05-04）。

### 結論

**モデル単体では全銘柄には通用しない。スクリーナー→モデルの2段階が必要。**

モデルは「スクリーナーが通過させた銘柄の中でどれが良いか」を選別するためのもの。
全銘柄にかけると効果がなく、スクリーナーとの組み合わせで初めてアルファが出る。

※注意: Alpha分析はスクリーナーCSVが2026-04-12時点のものを使用しているため、先読みバイアスが残る可能性あり。

---

## モデル改善履歴

### スクリーナー特化モデル実験（2026-05-07）— 不採用

**実験内容**

`rf_train_v3.py --screener-only` フラグを追加し、v1スクリーナーを通過した時点のサンプルのみで学習した専用モデル（`rf_model_screened.pkl`）を作成。`backtest.py --compare --screened` で通常モデルと比較。

**結果（通常モデル vs 特化モデル）**

| 期間 | 設定 | 通常モデル | 特化モデル |
|---|---|---|---|
| 通常期（日経+8.76%） | top5 | -1.56% (α-10.3%) | +11.93% (α+3.2%) |
| 通常期（日経+8.76%） | top10 | **+9.50% (α+0.7%)** | +5.80% (α-3.0%) |
| 下落相場（日経-2.47%） | top5 | -3.75% (α-1.3%) | -0.00% (α+2.5%) |
| 下落相場（日経-2.47%） | top10 | +20.83% (α+23.3%) | **+23.41% (α+25.9%)** |

**不採用の理由**

- AUCが大幅に低下（上昇: 0.544 / 下落: 0.609）— 学習サンプルが激減し汎化できていない
- `net≥10` 以上の閾値で使える銘柄が激減（2〜3銘柄）— 閾値フィルターが機能しない
- top5/top10 の優位は1〜2期間のみのデータで過学習の疑いが強い
- 実質「同じ銘柄のランキング順が僅かに違うだけ」の差

**追加されたCLIオプション（通常モデルでも利用可）**

- `backtest.py --top-n N` / `--net-min X`: 選択条件をCLIで調整
- `backtest.py --compare`: top-N × net-min のグリッドサーチ表を一括出力
- `backtest.py --screened`: `rf_model_screened.pkl` でバックテスト

---

### v7（2026-05-05）AUC 上昇0.663 / 下落0.791 ← 現在（キャリブレーション済み: 0.663/0.790）

**Task5〜7 システム改善（2026-05-05）**
- Task5: 確率キャリブレーション（Isotonic Regression）を追加
  - 学習データ最新20%をキャリブレーション用に分離、CalibratedClassifierCV(method="isotonic")で補正
  - AUCは変化なし（キャリブレーションは確率値を補正するが順位は変えない）
  - 効果: `上昇確率40%` が実際に約40%の銘柄で上昇するよう補正。ネットスコアの意味が明確化
  - `task5_calibrate.py`: 再学習不要で既存モデルにキャリブレーションを適用するスクリプト
- Task6: ファンダメンタル特徴量（PER/PBR/ROE）の表示を追加
  - `utils.get_fundamentals(code)`: kabutan.jpからPER/PBR/ROEをスクレイピング（Yahoo Finance v10は401）
  - `rank_stocks.py`: ランキング表示とCSVにPER/PBR/ROE列を追加
  - 注意: 訓練時は先読みバイアス回避のため未使用。推論時（ランキング）のみ表示
- Task7: サバイバーシップバイアス確認（`check_survivorship_bias.py`）
  - 推定欠損銘柄: 約298銘柄（全上場銘柄の約6.9%）
  - 下落ラベル率バイアス: 8.4% → 実態推定9.0%（+0.6%pt 過小評価）
  - 倒産・整理銘柄（約69社）のデータ取得は無料ソース不可 → 文書化で対応
  - 対策実施済み: down_streak/drawdown60フィルター・scale_pos_weightで間接的に軽減

### v7（2026-05-05）AUC 上昇0.663 / 下落0.791

**変更したこと**
- クロスセクショナルランク特徴量を追加（28→34次元）
  - 同日内の全銘柄を対象に ret5/ret20/ret60/rsi/vol20/pos52 をパーセンタイルランク化
  - 市場レジームをまたいだ絶対値のズレを吸収し、相対的な強さを捉える
- 両モデルとも改善（上昇+0.006、下落+0.001）
- Purged CV: 上昇0.628±0.025 / 下落0.782±0.036

### v6（2026-05-05）AUC 上昇0.657 / 下落0.790

**変更したこと**
- 60日リターン系列（60次元）を7次元要約統計量に圧縮（特徴量: 81→28次元）
  - autocorr_lag1, skew, max_ret, min_ret, pos_ratio, trend_slope, recent_vs_early
- 両モデルとも改善（上昇+0.005、下落+0.007）
- Task1: Purged K-Fold検証パイプライン（validation.py）を追加

### v5（2026-05-05）AUC 上昇0.655 / 下落0.782

**変更したこと**
- XGBoostハイパーパラメータ調整: n_estimators 500→800、learning_rate 0.05→0.04、early_stopping_rounds 30→50、min_child_weight 10→8
- サンプル統計ログ追加（学習/テスト各ラベル件数と比率）
- 試行して失敗した変更: 学習期間2500日拡張（コロナ期データがノイズになりAUC低下）、日経相対ラベル（AUC大幅低下→撤回）

**システム改善（2026-05-04〜05）**
- `backtest.py`: `requests`/`io` import追加、`FETCH_DAYS`を動的計算に変更（bearモードのデータ不足バグを修正）
- `rank_stocks.py` / `alert_email.py`: 日経20日リターン-5%以下で下落相場警告を表示（`BEAR_MARKET_THRESHOLD=-5.0`）。メール件名に【⚠️下落相場】付加
- `analyze_alpha.py`: スクリーナーとモデルのアルファ貢献度を分離する分析スクリプトを新規作成

### v4（2026-04-30）AUC 上昇0.644 / 下落0.780

**追加した特徴量（81次元）**
- 出来高3次元：vr520（5日/20日平均出来高比）、vr2060（20日/60日）、vsurge（直近/20日）
- 日経マクロ3次元：日経225の5/20/60日リターンを各サンプル時点で付加

**変更したこと**
- 出来高・日経マクロを入力特徴量として追加 → AUC改善
- 相対リターンラベル（日経比）を試みたが精度低下のため撤回
- 予測期間1ヶ月（21日）を試みたが精度低下のため3ヶ月（63日）に戻した
- early_stopping_rounds=30 をコンストラクタに追加（上昇88エポック、下落168エポックで停止）

### v3（2026-04-28）AUC 上昇0.603 / 下落0.767

- TSE全銘柄（プライム・スタンダード）× 5年データで学習
- ウォークフォワード分割導入（2025-01-01で学習/テスト分離）
- 20営業日ごとサンプリング（自己相関低減）
- 75次元特徴量（テクニカル10 + トレンド反転5 + 60日系列）

### v2以前

- スクリーナー通過銘柄のみで学習 → 先読みバイアスあり
- ランダム500銘柄サンプリング → サンプル数不足
