...
| `web/market_timing_alert.py` | LINE Messaging APIで日次プッシュ通知（Step 5b）。N225シグナル（平均下落確率→投資/キャッシュ）・🌐日経 vs S&P500相対強弱・🏦直近のEDINET大口保有動向（自己申告/譲渡・売却を除外、直近3日・最大5件）・ユーザー別ウォッチリストのdp閾値アラートを配信 |
...
| `tests/test_market_timing_alert.py` | LINE通知の大口保有動向セクション整形のユニットテスト（5件）|
