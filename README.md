...
**件数や集合そのものが判断材料になる呼び出し**（ランキング・記事化候補・重複判定・通知先・集計レポート・
バックフィルの対象抽出）は2026-09-10に全て strict にした。strict にしていない呼び出し（表示用・
1リクエストで終わる limit 付き・冪等な修正）には、その場に「なぜ安全か」をコメントで残している。
新しく `sb.select()` を足すときも同じ基準で選ぶこと。大きいテーブル（`blog_crawler_log` 42万行・
`yahoo_price_cache` 157万行・`gen_rankings` 39万行）を広い範囲で引く箇所は、strict にするだけでなく
主キーや時刻で小分けにする（offset が深いと statement timeout(8秒) に当たる。`blog_crawler_log` の
30日ぶん＝約21万行は offset 20万で1ページ6.8秒＝2026-09-10実測）。
...
| `web/market_timing_alert.py` | N225シグナル・日経 vs S&P500相対強弱・EDINET大口保有動向・ウォッチリストをLINE Messaging APIへ送る手動実行用スクリプト。日次プッシュ通知（旧Step 5b）は2026-09-02に停止。コードは調査・手動実行用に残している |
...
| `tools/traffic_report.py` | **アクセスログの機械/人の切り分け**（手動実行専用）。`blog_crawler_log`の"Browser"判定（既知botのUAでなくブラウザUAのもの）には機械が大量に混ざるため、4段階で除外して残りを表示する（UAは`blog_crawler_ua`へ正規化されているので`ua_id`から引き当てる。ログは1日ずつの窓に分けて`strict=True`で引く＝30日ぶん約21万行を1本でページングするとoffset 20万で1ページ6.8秒とstatement timeout(8秒)の手前まで遅くなり、途中で切れた行数で集計すると比率が静かに狂うため）: ①**self**=自サイトからの自己リクエスト（`::1`/`127.0.0.1`）②**bot_ua**=UAに`bot`/`crawler`/`spider`/`externalagent`/`+URL`を含む自己申告（`crawlers.ts`の`BOT_PATTERNS`に載っていない新顔をここで拾い、名前を集計して表示するので登録漏れに気づける）③**heavy_ip**=1IPあたりPVが閾値（`--max-pv-per-ip`、既定100）超 ④**cookieless_ua**=同一UAで`--min-ua-pv`（既定20）以上のPVがあるのに`visitor_id`がPVとほぼ1:1（比0.9以上）＝クッキーを保持しない機械。残りのPV・IP数・2PV以上見た訪問者数・JST時間帯分布・記事ページ率・人気pathを表示する。**実測2026-08-28（1日）**: Browser 3,573PVの内訳がself 187 / bot_ua 1,384（meta-externalagent 1,031・Amazonbot 337）/ heavy_ip 1,421 / cookieless_ua 525で、残った人の候補は50〜100PV。同日の全リクエスト19,658件のうち81.8%はUAでbotと名乗っており、最大はGoogleOtherの13,497件。クッキー(visitor_id)の**有無**とJS実行(`/api/counter`)の有無は判別に使えないことを実データで確認済み（どちらも1人あたり74〜75PVでクローラーが同条件を満たす）ため、同じ検証を繰り返さないようdocstringに記録している（cookieless_uaは「有無」ではなく「PVに対する発行数の比」で見るので別物） |
