...
| `MICROCMS_SERVICE_DOMAIN` | `kujira-watch`（大口投資家の監視ブログ）用microCMSサービスドメイン（Step 5c: ブログ記事自動投稿。未登録ならスキップ）|
