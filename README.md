...
| `tools/daily_log_review.py` | **日次ログレビュー（AIフィードバック）**（daily_log_review.yml、平日0:30 JST）。GitHub REST API（`gh api .../actions/runs`）で直近24hの全ワークフロー（自分自身を除く。ci.yml は失敗runのみ。ファイル`path`で束ねるので ops.yml の Keepalive/Watchdog のようにジョブ名で表示される run も漏れない）の実行結果とログを取得し、エラー・警告・記事タイトル・投稿/見送り行と各ステップの先頭・末尾だけに圧縮（`condense_log`。LINE送信本文を含むマーケットタイミングのステップは全文保持）。さらにSupabaseから成果物スナップショット（X投稿文と反応・フォロワー推移・`blog_crawler_log`の人間推定PV＝`bot_name='Browser'`かつ1IP≤100PV/8日・上位ページ・当日ランキングの推奨ラベル内訳）を付けてClaude（claude-opus-5）に渡す。SRE／クオンツ運用／プロダクト／SEO・GEO／UX・デザイン／バックエンド／フロントエンド／PdM の8観点で「健全性サマリー／異常・注意点／SEO・GEO所見／UX・デザイン所見／エンジニアリング所見／PdM所見（KPIと今週やる3件・やらない事）／改善提案（観点名付き）／LINE要約」をMarkdownで書かせ、全文は`daily-review`ラベルのIssue1本にコメント追記＋Actionsジョブサマリー、「LINE要約」節だけをLINEへpush。`--dry-run`で標準出力のみ、`--since-hours N`で対象期間変更。1回あたり入力15〜20万字・API費用は概ね$1前後 |
| `tests/test_daily_log_review.py` | 日次ログレビューのユニットテスト（ログ圧縮の残す行/落とす行・全文保持ステップ・サイズ上限・run絞り込み・path別グルーピング(CIは失敗のみ)・成果物スナップショットの人間PV判定と整形・LINE要約抽出・メッセージ組み立て、11件）|
