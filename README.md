...
**推奨ラベル**:
- 🥇 S買い: 下落確率 < 4% かつ ネット ≥ 10%（低リスク高期待値。大勝ち銘柄の平均drop=3.2%より設定）
- 🥈 A買い: ネット ≥ 10%（標準推奨）
- ⏳ 方向感なし: ネット 6〜10%（様子見。新規候補には表示されない）
- 🔴即切: 20日リターン < −10%（損切りライン割れ）

**S買いシグナルの追加フィルター（rank_stocks.py フェーズ5〜7）**:
- フェーズ5: 決算21日以内の銘柄はA買いに降格
- フェーズ6: 1日最大3件のキャップ（net降順）
- フェーズ7: 対応する米国セクターETF（XLK/XLF/XLI/XLB/XLV/XLY）の前日リターンがマイナスならA買いに降格。リードラグ効果（US→JP翌日）を活用。21,416サンプル(2023-2026)で全26ペア正相関・avg +0.64pp効果を確認。キャッシュは `data/sector_map.json`。
