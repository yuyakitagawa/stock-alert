...
- `web/generate_descriptions.py`：**2段階生成**で事実精度を重視。
  ...
  → Supabase `claude_ai_analyses`(model_version=`company-desc-v1`) に保存。
...
- API `/api/stock/[code]/insight`（Claude Haiku、Supabase `claude_ai_analyses` の model_version=`company-insight-v1` にJSONキャッシュ）→ `components/StockInsightPanel.tsx` が「🤖 AI生成・参考」免責付きで表示。取引先は要確認（一次情報＝有価証券報告書）の注記あり。
...
| `web/generate_descriptions.py` | 全銘柄の会社説明を2段階で生成（Phase1: Yahoo特色スクレイプ／Phase2: Haiku Haikuフォールバック）→ `claude_ai_analyses`(company-desc-v1)。`--refresh` で全銘柄再スクレイプ。Step 5a2 |
| `web/sync_descriptions.py` | スプシ「📝 会社説明」の手動説明を `claude_ai_analyses`(company-desc-v1) へ同期（AI生成を上書き）。Step 5b |
...
| `lib/db.py` | Supabase永続化層（web_rankings / kabutan_earnings / web_stock_meta / yahoo_price_cache ほか）。`lib/supabase_client.py` のREST API経由 |
...
保有日数は Google Sheets（チェック銘柄一覧）の**購入日列**から算出する。購入日が未設定の銘柄は保有日数 `None` 扱いで、モデル外の段階引き締めは適用しない。
...
| `kabutan_jquants_margin` / `jpx_short_interest` / `tdnet_events` | 信用・空売り・適時開示（週次/日次）|
