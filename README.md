...
| `web/market_timing_alert.py` | LINE Messaging APIで日次プッシュ通知（Step 5b）。N225シグナル（平均下落確率→投資/キャッシュ）・🌐日経 vs S&P500相対強弱・🏦直近のEDINET大口保有動向（自己申告・過半数超(51%以上、スクイーズアウト対象で上値が見込めない)は除外、譲渡/売却も📈買い・📉売りを明示して表示。同一提出者の開示が複数あれば保有比率の変化を「5.2%→10.1%」で表示（訂正報告書は事後修正のため除外。直近ウィンドウ内に1件しか無い提出者でも、ウィンドウより前の直近の実際の開示と比較して変化を表示）。ウォッチ銘柄→法人/ファンド→保有比率が大きい順に優先し最大5件、個人名の提出者は後回し。残りはLINEで「大量保有」「大口」と聞けばAIの要約を介さず`check_catalyst`を直接呼び出し全件を返す（要約による省略防止））・ユーザー別ウォッチリストのdp閾値アラートを配信 |
...
| `tests/test_market_timing_alert.py` | LINE通知の大口保有動向セクション整形のユニットテスト（14件）|
| `tests/test_scan_large_holdings.py` | EDINET大量保有スキャナーの判定ロジック（売却検知・個人名判定・過半数超除外・訂正報告書除外・ノイズ除外）のユニットテスト（9件）|
