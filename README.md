...
| `tests/test_x_filer_record.py` | 提出者ごとのリターン実績（`web/x_filer_record.py`）のユニットテスト（旧字体「證券」を含む自己名義提出者を除外すること・休場は許容し長期の欠落は使わないこと・株式分割で不連続な銘柄を弾くこと・両端の終値が揃わない開示を落とすこと・10件未満の提出者を出さないこと・週ごとに別の提出者を選ぶこと・本文に全開示平均を必ず入れること・負けている提出者を「上」と書かないこと）（8件、SupabaseとX APIは呼ばない）|
