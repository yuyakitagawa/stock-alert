...
（大口投資家の俗称）の動きを解説する。SEO/AIO対策・独自ドメイン対応済み。2026-09-10に、開示の読み方を1章から順に学べる**教科書**（`/textbook` 全6章。章末に最新の開示を実例として出すので、
本文は固定でも実例は毎日のクロールで入れ替わる）と、提出期限・自社株買いインパクトを自分の数字で計算できる
**ツール**（`/tools`。ブラウザ内で完結し、入力値もAnthropic APIも使わない）を追加した。
詳細は `kujira-watch/README.md` を参照。
