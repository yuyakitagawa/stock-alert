...
def get_tse_stock_list():
    """JPXから全上場銘柄を取得（内国株式 + ETF/ETN、REIT除外）"""
    ...
    try:
        ...
        if market_col:
            market_filter = df[market_col[0]].str.contains("内国株式|ETF|ETN", na=False)
            result = result[market_filter]
            result = result[result["code"].str.match(r"^\d{3}[0-9A-Z]$")]
            print(f"   市場区分フィルター適用（内国株式+ETF/ETN）")
        else:
            result = result[result["code"].str.match(r"^\d{3}[0-9A-Z]$")]
