...
| `/ranking/[slug]` | 月間ランキング。直近30日の開示ランキング3種（SEO/AIO 30日計画P2、2026-08-15新設）。ヘッダー・フッターの「月間ランキング」はこの`buys`が入口（旧`/ranking`＝3ヶ月リターンランキングは推定損益の算出が誤っていたため2026-08-18に廃止し、`next.config.ts`で`/ranking`→`/ranking/buys`へ恒久リダイレクト）。`buys`=買い増し／`sells`=売却は**投資家別**の集計（2026-08-18修正。h1がランキングのタブ見出しなのに銘柄別に並べていて、タブを切り替えると軸が銘柄に変わってしまっていた）。どちらも投資家ごとの推定金額の合計降順（同額は開示件数降順）。提出者名(`filerName`)が無い過去記事は投資家別に積めないため集計対象外。各行は代表開示（金額が最大の開示）の銘柄名・解説記事をメタとして添え、投資家ページ・銘柄ページ・解説記事の3リンクを張る（リンクが3つあるためカード全体はリンクにしない）。`activist`=アクティビスト分類の開示（金額降順）だけは「アクティビストが動いた銘柄」でタブにも含めないため銘柄別のまま。集計は`src/lib/rankingStats.ts`（`buildFilerRows()`/`buildStockRows()`）、データ元は`getRecentArticles(30)`（microCMS記事）。表示は「1行目=順位＋投資家名（全幅）／2行目=分類・件数・代表銘柄・金額（右端）」の1件1カード（`.card-grid-wide`＝2列相当）。上位30件固定なのでページ送りは無い。`generateStaticParams`で3slugを事前生成、`revalidate=3600`、BreadcrumbList+ItemListのJSON-LD（投資家別ランキングのItemListは投資家ページを指す）とsitemap登録済み。記事詳細ページの「関連ランキング」navからも導線あり。各ページ上部に`RankingTabNav`のタブを表示（activistはタブ非選択）。タブ切替時に見出しの位置がずれないよう、h1は全ランキングページ共通の「月間ランキング」とし、個別ランキング名はタブ直下のh2に置く（2026-08-18）。`filings`（報告書件数ランキング）は2026-08-18に廃止（直近30日の開示件数を投資家別に数えるだけで、`/ranking/trending`の「前30日比で増えた投資家」と対象がほぼ重なり、常連の提出者が並ぶだけで読み手の判断材料にならなかった）。インデックス済みURLなので404にせず`redirects()`で`/ranking/trending`へ301 |
