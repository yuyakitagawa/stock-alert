...
| `/activists` | アクティビストの動き（2026-08-18に「保有銘柄一覧」から改名・再構成）。「アクティビスト注目銘柄」（直近30日にアクティビストが保有比率を増やした＝買い入れた銘柄を、増加幅(pt)の銘柄別合算が大きい順に初期10件＋`<details>`の「もっと見る」。前回比率が無い開示は新規保有として今回比率ぶんを増加扱い。2ファンド以上が同時5%以上保有中の銘柄には「複数ファンド保有」バッジ。定義の解説は`/faq/usage`のQ&Aへ）→「直近のアクティビストの動き」（`getActivistRecentMoves()`＝開示を新しい順に初期20件＋「もっと見る」、`RatioTransition`の前回→今回つき）の2段構成。以前あった「ファンド別の保有銘柄」セクションは削除し、各投資家ページ（`/investors/[filer]`）の「主な保有銘柄」に集約（ファンド一覧のItemList構造化データも削除）。保有集計は`src/lib/activists.ts`の`getActivistHoldingsSummary()`（複数ファンド保有バッジの判定に使用。提出者名を15件ずつ`.in()`分割で問い合わせ、(提出者×銘柄)の最新行を採用）。銘柄は記事がある銘柄のみリンク（`/trending`と同じ規律）。提出者名は表示のみ`displayFilerName()`で半角へ寄せ、hrefは原文のまま。`unstable_cache`で1時間キャッシュ、ヘッダーから常時リンク |
...
| `/ranking` | 投資家ランキング（2026-08-18に「投資家別 3ヶ月勝率ランキング」から改名）。3ヶ月リターン（本ページ）／買い増し／売却／報告書件数／開示急増投資家の5ランキングを`RankingTabNav`（`FilterButtonNav`のタブ）で切り替える。デフォルトの3ヶ月リターンランキングは`tools/filer_win_rate.py`が週次（GitHub Actions `filer_win_rate.yml`）で再計算するSupabase `filer_win_rate`テーブルを`lib/investors.ts`の`getFilerWinRates()`が収縮後勝率(shrunk_win_rate)降順で取得。買い開示件数(n)が5未満の投資家は表示しない（サンプル不足で勝率のブレが大きいため）。`/investors`と同じ`?category=`クエリでのカテゴリ絞り込みに対応（分類が13種類あってランキング本体が下へ押し下げられるため、`FilterButtonNav`の`collapsible`で`<details>`に折り畳み、既定は閉じた状態。絞り込み中のときだけ「カテゴリで絞り込む（現在: アクティビスト（26件））」と現在の絞り込みを見出しに出して開いた状態で描画する）。表示はテーブルではなく「順位＋投資家名／分類バッジ・件数／リターン」の1件1行リスト（全画面幅共通）。5列テーブルだと狭い画面で投資家名が1文字ずつ折り返されて読めなくなるため。説明文は2文＋FAQ（算出方法・除外条件のQ&A）へのリンクに短縮（2026-08-18）。アクティビストランキング（`/ranking/activist`）はタブに含めず、`/ranking/activist`⇄`/activists`の相互リンクと`/activists`からのみ誘導（他タブページには無関係なリンクを置かない） |
