...
| `/ranking` | 投資家ランキング（2026-08-18に「投資家別 3ヶ月勝率ランキング」から改名）。3ヶ月リターン（本ページ）／買い増し／売却／報告書件数／開示急増投資家の5ランキングを`RankingTabNav`（`FilterButtonNav`のタブ）で切り替える。デフォルトの3ヶ月リターンランキングは`tools/filer_win_rate.py`が週次（GitHub Actions `filer_win_rate.yml`）で再計算するSupabase `filer_win_rate`テーブルを`lib/investors.ts`の`getFilerWinRates()`が収縮後勝率(shrunk_win_rate)降順で取得。買い開示件数(n)が5未満の投資家は表示しない（サンプル不足で勝率のブレが大きいため）。`/investors`と同じ`?category=`クエリでのカテゴリ絞り込みに対応。表示はテーブルではなく「順位＋投資家名／分類バッジ・件数／リターン」の1件1行リスト（全画面幅共通）。5列テーブルだと狭い画面で投資家名が1文字ずつ折り返されて読めなくなるため。説明文は2文＋FAQ（算出方法・除外条件のQ&A）へのリンクに短縮（2026-08-18）。アクティビストランキング（`/ranking/activist`）はタブに含めず、`/ranking/activist`⇄`/activists`の相互リンクと`/activists`からのみ誘導（他タブページには無関係なリンクを置かない） |
