...
| `/ranking/trending` | 開示急増投資家ランキング（2026-08-18新設）。直近30日にEDINETへ提出された大量保有・変更報告書の件数を投資家別に集計し、その前の30日と比べて増加件数の多い順に上位30件（2026-08-18に10件から拡大。/trendingの銘柄版は10件のまま）（`buildTrendingFilers()`＋`TrendingTable`共用コンポーネント。従来`/trending`にあった「開示が増えた投資家」表の移設先）。`RankingTabNav`のタブに含まれる。lastmodは最新開示日 |
| `/trending` | 取引が急増した銘柄（2026-08-18に「銘柄・投資家」の2表構成から銘柄のみに変更し「クジラが急増した銘柄」→「取引が急増した銘柄」に改名。投資家版は`/ranking/trending`へ移設）。ページ冒頭の説明文は2文＋注目投資家・FAQへのリンクのみに絞り、NEWの意味・件数と金額の違いなどの補足は`/faq/usage`のQ&Aへ移設。直近30日間にEDINETへ提出された大量保有・変更報告書の件数を、その前の30日間と比べて増加件数の多い順にランキング（銘柄別、上位10件。`src/lib/trendingStats.ts`＋`TrendingTable`共用コンポーネント。MUI Tableは`minWidth: 420px`で375px幅だと横スクロールが必要だったため、1件1カード（`.card-grid-wide`）に変更し各数値にラベルを添えている）。集計元は記事(microCMS)ではなくSupabase `edinet_large_holdings`（`getHoldingsInRange()`）。記事の蓄積は2026年7月に始まったばかりで「前30日間」がほぼ空になり比較が成立しないのに対し、開示データは1年分あるため今日時点でも意味のある前期間比が出せる。開示データに推定取引金額は無いため、比較軸は金額ではなく開示件数（本文にもその旨を明記）。ランキング表の下に「月別の開示件数トレンド」の棒グラフ（`src/components/DisclosureTrendChart.tsx`（月別・週別共用）＋`src/lib/disclosures.ts`の`getMonthlyDisclosureCounts()`。株価グラフと同じくチャートライブラリ無しのインラインSVG。単一系列なので凡例なし・ネイビー1色、当月は薄色で「集計中」表示、数値ラベルは最大値と最新月のみ、各棒のSVG `<title>`でホバー時に月と件数、`<details>`内の表で全数値をクローラー・スクリーンリーダーからも読めるようにしている。データ取得開始月＝2025-06は月の途中からで件数が過少のため除外）を表示。各カードには社名の下に事業内容の1文（`jpx_stock_list.description`、`getCompanyBriefs()`で表示中の10銘柄ぶんだけ一括取得）を添える。社名と証券コードだけでは何の会社か分からず判断できないため（2026-08-18追加）。事業内容が未生成の銘柄は業種（`sector`、ほぼ全銘柄にある）で代替する。事業内容の行数差でカード下端がガタつかないよう、カードを`flex h-full flex-col`で行の高さいっぱいに伸ばし件数行を`mt-auto`で下揃えにしている。事業内容そのものの生成はトレーディングシステム側（`web/publish_blog_articles.py`の`get_company_description()`＝web検索で裏取り、`tools/backfill_company_descriptions.py --recent-days N`で既存銘柄を一括バックフィル）。銘柄ページ(`/stocks/[code]`)は上場銘柄マスターに載っていれば解説記事が無くても存在するため、マスターに無いコード（上場廃止等）だけリンクにせずテキストのまま出す（404へのリンクを作らない）。ヘッダーから常時リンク |
