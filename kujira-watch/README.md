...
| `/ranking/[slug]` | 投資家ランキング。直近30日の開示ランキング4種（SEO/AIO 30日計画P2、2026-08-15新設）。ヘッダー・フッターの「投資家ランキング」はこの`buys`が入口（旧`/ranking`＝3ヶ月リターンランキングは推定損益の算出が誤っていたため2026-08-18に廃止し、`next.config.ts`で`/ranking`→`/ranking/buys`へ恒久リダイレクト）。`buys`=買い増し（推定取得金額降順）／`sells`=売却（推定売却金額降順）／`filings`=大量保有報告書の銘柄別件数（同数は合計金額順、代表記事は金額最大の開示）／`activist`=アクティビスト分類の開示（金額降順）。データは`getRecentArticles(30)`（microCMS記事）から集計し、各カードに銘柄ページ・投資家ページ・解説記事への内部リンクを張る（リンクが3つあるためカード全体はリンクにしない）。表示は「1行目=順位＋銘柄名（全幅）／2行目=メタ・金額（右端）」の1件1カード（`.card-grid-wide`＝2列相当）。上位30件固定なのでページ送りは無い。`generateStaticParams`で4slugを事前生成、`revalidate=3600`、BreadcrumbList+ItemListのJSON-LDとsitemap登録済み。記事詳細ページの「関連ランキング」navからも導線あり。各ページ上部に`RankingTabNav`のタブを表示（activistはタブ非選択）。タブ切替時に見出しの位置がずれないよう、h1は全ランキングページ共通の「投資家ランキング」とし、個別ランキング名はタブ直下のh2に置く（2026-08-18） |
...
- ヘッダー右上の🔍アイコン（`src/components/StockSearch.tsx`）から企業名・証券コードで検索できる。入力停止から300ms後に`/api/stocks/search`（`lib/microcms.ts`の`searchStocks()`＝記事の`stockCode`/`stockName`の`[contains]`部分一致、＋`lib/companyInfo.ts`の`searchStockMaster()`＝Supabase `jpx_stock_list`のコード前方一致・社名部分一致。記事側だけだと記事化されていない銘柄が検索から消えるため両方を引く。銘柄単位で重複排除・最大20件）を叩き、結果をクリック（またはEnterで先頭候補）すると`/stocks/[code]`（銘柄別履歴）に遷移する。
