...
| `/stocks` | 銘柄一覧。記事のある銘柄を最終開示日が新しい順に列挙（`lib/microcms.ts`の`getAllStocksForIndex()`） |
...
| `/faq` | よくある質問（FAQPage構造化データ付き、86問。各質問に色分けされたカテゴリバッジを表示し、5カテゴリのタブでも絞り込み可能）。大量保有報告書のしくみ・投資家分類・本サイトの使い方・初心者/中級者向けの読み方 |
