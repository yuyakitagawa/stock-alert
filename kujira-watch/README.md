...
| `/about` | 運営者情報・データソース・免責事項（E-E-A-T対策）。投資家分類の用語集（`#dealtype-glossary`）と公式X（@kujira_watch）の埋め込みも含む |
...
- **X（旧Twitter）埋め込み**: `/about` 冒頭に公式X（@kujira_watch）の最新1投稿を埋め込む（`src/components/XTimeline.tsx`）。`platform.twitter.com/widgets.js`を`next/script`の`lazyOnload`で遅延読み込みし、LCP/CLSへの影響を抑えている。`data-tweet-limit=1`で1件に絞り、`data-chrome="noheader nofooter noborders transparent"`でX標準の枠を消して自前のカード（負のマージンで記事枠から左右にはみ出し、`-rotate-1`で傾ける）に載せている。サイトがダークモード非対応のため`data-theme="light"`固定。
