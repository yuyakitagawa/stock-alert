...
- **一覧の件数とページ送り**: 1ページに100件を超えて描画しない。`/trending`と`/ranking/trending`はオートページャー（初回30件＋スクロールで30件ずつ）、`/disclosures`・`/stocks`・`/investors`は`?page=`のページ送り（`?page=`はクロール可能な素の前後リンク、各ページ自身がcanonical、絞り込みリンクはページ番号を持ち越さない）。カード化で1件あたりの縦幅が増えるため、行リスト時代より件数上限の意味が大きい。
