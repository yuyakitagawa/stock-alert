...
| `/ranking` | 月間ランキング（2026-08-18に「投資家別 3ヶ月勝率ランキング」→「投資家ランキング」→「月間ランキング」と改名。h1・パンくず・ヘッダー/フッターのナビ表記もこの名前で統一。タブ配下は直近30日の集計が主なので「月間」を冠しているが、既定タブの3ヶ月リターンだけは63営業日の評価である点に注意）。3ヶ月リターン（本ページ）／買い増し／売却／開示急増投資家の4ランキングを`RankingTabNav`（`FilterButtonNav`のタブ）で切り替える。デフォルトの3ヶ月リターンランキングは`tools/filer_win_rate.py`が週次（GitHub Actions `filer_win_rate.yml`）で再計算するSupabase `filer_win_rate`テーブルを`lib/investors.ts`の`getFilerWinRates()`が収縮後勝率(shrunk_win_rate)降順で取得。買い開示件数(n)が5未満の投資家は表示しない（サンプル不足で勝率のブレが大きいため）。`/investors`と同じ`?category=`クエリでのカテゴリ絞り込みに対応（分類が13種類あってランキング本体が下へ押し下げられるため、`FilterButtonNav`の`collapsible`で`<details>`に折り畳み、既定は閉じた状態。絞り込み中のときだけ「カテゴリで絞り込む（現在: アクティビスト（26件））」と現在の絞り込みを見出しに出して開いた状態で描画する）。表示はテーブルではなく「1行目=順位＋投資家名／2行目=分類バッジ・件数・リターン（右端）」の1件1カード（`.card-grid-wide`＝2列相当）。5列テーブルだと狭い画面で投資家名が1文字ずつ折り返されて読めなくなるため。リターンを名前と同じ行に置くと名前が細く折り返されるので2段に分けている。`?page=`で100件ずつのページ送り（順位はページをまたいで通し番号。各ページは自分自身をcanonical）。説明文は2文＋FAQ（算出方法・除外条件のQ&A）へのリンクに短縮（2026-08-18）。アクティビストランキング（`/ranking/activist`）はタブに含めず、`/ranking/activist`⇄`/activists`の相互リンクと`/activists`からのみ誘導（他タブページには無関係なリンクを置かない） |
| `/ranking/[slug]` | 直近30日の開示ランキング3種（SEO/AIO 30日計画P2、2026-08-15新設）。`buys`=買い増し／`sells`=売却は**投資家別**の集計（2026-08-18修正。h1が「月間ランキング」でタブの一員なのに銘柄別に並べていて、タブを切り替えると軸が銘柄に変わってしまっていた）。どちらも投資家ごとの推定金額の合計降順（同額は開示件数降順）。提出者名(`filerName`)が無い過去記事は投資家別に積めないため集計対象外。各行は代表開示（金額が最大の開示）の銘柄名・解説記事をメタとして添え、投資家ページ・銘柄ページ・解説記事の3リンクを張る（リンクが3つあるためカード全体はリンクにしない）。`activist`=アクティビスト分類の開示（金額降順）だけは「アクティビストが動いた銘柄」でタブにも含めないため銘柄別のまま。集計は`src/lib/rankingStats.ts`（`buildFilerRows()`/`buildStockRows()`）、データ元は`getRecentArticles(30)`（microCMS記事）。表示は`/ranking`と同じ「1行目=順位＋投資家名（全幅）／2行目=分類・件数・代表銘柄・金額（右端）」の1件1カード（`.card-grid-wide`＝2列相当）。上位30件固定なのでページ送りは無い。`generateStaticParams`で3slugを事前生成、`revalidate=3600`、BreadcrumbList+ItemListのJSON-LD（投資家別ランキングのItemListは投資家ページを指す）とsitemap登録済み。記事詳細ページの「関連ランキング」navからも導線あり。各ページ上部に`RankingTabNav`のタブを表示（activistはタブ非選択）。タブ切替時に見出しの位置がずれないよう、h1は全ランキングページ共通の「月間ランキング」とし、個別ランキング名はタブ直下のh2に置く（2026-08-18）。`filings`（報告書件数ランキング）は2026-08-18に廃止（直近30日の開示件数を投資家別に数えるだけで、`/ranking/trending`の「前30日比で増えた投資家」と対象がほぼ重なり、常連の提出者が並ぶだけで読み手の判断材料にならなかった）。インデックス済みURLなので404にせず`next.config.ts`の`redirects()`で`/ranking/trending`へ301 |
...
- **一覧の件数とページ送り**: 1ページに100件を超えて描画しない。`/trending`と`/ranking/trending`はオートページャー（初回30件＋スクロールで30件ずつ）、`/disclosures`・`/stocks`・`/ranking`・`/investors`は`?page=`のページ送り（`?page=`はクロール可能な素の前後リンク、各ページ自身がcanonical、絞り込みリンクはページ番号を持ち越さない）。カード化で1件あたりの縦幅が増えるため、行リスト時代より件数上限の意味が大きい。
