...
| `/activists` | アクティビストの動き（2026-08-18に「保有銘柄一覧」から改名・拡充）。冒頭に「直近30日の動き」（`getActivistRecentMoves()`＝アクティビストの開示を新しい順に最新50件、`RatioTransition`の前回→今回つき。50件超は開示速報への誘導注記）を置き、続けて投資家分類「アクティビスト」の提出者について、EDINET開示の銘柄ごとの最新開示を「現在の保有」として横断集計（`src/lib/activists.ts`の`getActivistHoldingsSummary()`。提出者名51件を15件ずつ`.in()`分割で問い合わせ、(提出者×銘柄)の最新行を採用。最新の保有比率が5%未満＝報告義務外に下がった銘柄は以降の売買が開示されないため除外）。「複数のアクティビストが保有する銘柄」（2ファンド以上が同時保有、ファンド数→最大比率順）→「ファンド別の保有銘柄」（保有銘柄数順、`/investors/[filer]`へリンク）の2段構成で、各行に保有比率と最終開示日を表示。銘柄は記事がある銘柄のみリンク（`/trending`と同じ規律）。提出者名は表示のみ`displayFilerName()`で半角へ寄せ、href/JSON-LDは原文のまま。競合（アクティビストメディアの保有一覧）に対応しつつ、`/ranking/activist`（直近30日のフロー）と補完関係にある「ストック側」のページ。`unstable_cache`で1時間キャッシュ、ヘッダーから常時リンク |
