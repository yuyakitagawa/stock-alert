...
| `/trending` | クジラが急増した銘柄（2026-08-18に「銘柄・投資家」の2表構成から銘柄のみに変更。投資家版は`/ranking/trending`へ移設）。直近30日間にEDINETへ提出された大量保有・変更報告書の件数を、その前の30日間と比べて増加件数の多い順にランキング（銘柄別、上位10件。`src/lib/trendingStats.ts`＋`TrendingTable`共用コンポーネント）。集計元は記事(microCMS)ではなくSupabase `edinet_large_holdings`（`getHoldingsInRange()`）。記事の蓄積は2026年7月に始まったばかりで「前30日間」がほぼ空になり比較が成立しないのに対し、開示データは1年分あるため今日時点でも意味のある前期間比が出せる。開示データに推定取引金額は無いため、比較軸は金額ではなく開示件数（本文にもその旨を明記）。ランキング表の下に「月別の開示件数トレンド」の棒グラフ（`src/components/MonthlyDisclosureTrend.tsx`＋`src/lib/disclosures.ts`の`getMonthlyDisclosureCounts()`。株価グラフと同じくチャートライブラリ無しのインラインSVG。単一系列なので凡例なし・ネイビー1色、当月は薄色で「集計中」表示、数値ラベルは最大値と最新月のみ、各棒のSVG `<title>`でホバー時に月と件数、`<details>`内の表で全数値をクローラー・スクリーンリーダーからも読めるようにしている。データ取得開始月＝2025-06は月の途中からで件数が過少のため除外）を表示。銘柄ページ(`/stocks/[code]`)は記事のある銘柄にしか存在しないため、記事の無い銘柄はリンクにせずテキストのまま出す（404へのリンクを作らない）。ヘッダーから常時リンク |
