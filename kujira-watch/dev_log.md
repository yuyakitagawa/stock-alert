## 2026-09-10 kujira-watch: 教科書（/textbook 全6章）と計算ツール（/tools）を新設

オーナー指示「AIが最新記事をクロールし、教科書とツールを進化させるサイトにしたい」。
クロール→記事生成までは動いていたが、(1)制度を知らない読者が順に読める入口が無い
（FAQ 502件は疑問の語を知っている人しか引けない）、(2)記事の数字を自分の持ち株でやり直す手段が無い、
の2点が欠けていた。

- `src/lib/textbook.ts`（新規）: 全6章の本文データ。`basics`（制度の目的）/ `rules`（提出義務と期限）/
  `reading`（報告書の読み方）/ `players`（提出者の分類）/ `buyback`（自社株買い）/ `practice`（仮説と検証）。
- `src/app/(ja)/textbook/page.tsx`・`[chapter]/page.tsx`（新規）: 目次と章ページ。章末に
  **その章の実例**として最新記事3件を `getArticleList()` で出す（`reading`はアクティビスト、
  `buyback`は自社株買いで絞る）。本文は固定だが実例は毎日のクロールで入れ替わる（`revalidate = 3600`）。
  microCMSが落ちても本文は出す（実例だけ握りつぶし）。構造化データは `Course` + `LearningResource`。
- FAQの回答本文は章に複製せず、関連FAQカテゴリへのリンクだけ置く（重複コンテンツを作らない）。
- `src/lib/marketCalendar.ts`（新規）: 国民の祝日に関する法律の規定（固定日・ハッピーマンデー・
  春分秋分の近似式・振替休日・国民の休日）から祝日を算出し、行政機関の休日（土日・祝日・12/29〜1/3）を
  除いて営業日を数える。**年ごとの祝日表は持たない**（表は更新が止まった瞬間に黙って誤った期限を出す）。
- `src/lib/filingDeadline.ts`（新規）: 提出義務の判定（新規＝法27条の23第1項の5%超 / 変更＝法27条の25第1項の
  1%ルール / 5%以下になった旨 / 義務なし）と、初日不算入で数えた提出期限。特例報告（法27条の26）の
  基準日（毎月15日・月末）も返す。
- `src/lib/buybackImpact.ts`（新規）: 取得価額の上限と取得株数の上限のうち先に当たる方で実際の取得株数を決め、
  発行済比率・時価総額比・EPS押し上げ率（1/(1−比率)−1）・出来高何日ぶんかを計算。
- `src/lib/tools.ts`（新規）: ツールの台帳。`src/app/(ja)/tools/`（ハブ + `filing-deadline` + `buyback-impact`）と
  `src/components/tools/`（入力フォーム、`"use client"`）。**入力値はサーバーに送らない**
  （保有割合は公開前の情報になりうるため）。Anthropic APIも使わない＝追加の従量コストゼロ。
- `src/lib/nav.ts`・`src/components/Footer.tsx`・`src/app/sitemap.ts`: 導線を追加。ヘッダー/フッター/
  サイトマップは `textbook.ts` と `tools.ts` の台帳だけを見るので、章やツールを足しても追加漏れが起きない。

検証: `npx tsc --noEmit`・`npx eslint`・`python3 tools/check_design_system.py` パス。
祝日算出は2025年（19日）・2026年（18日）・2027年（17日）を出力して官報の祝日と突合し一致を確認
（2026年のシルバーウィーク 9/21敬老の日・9/22国民の休日・9/23秋分の日、2026年の5/6振替休日を含む）。
自社株買い計算は 上限50億円 / 300万株 / 発行済5,000万株 / 株価1,800円 / 純利益120億円 / 出来高20万株 で
「金額の上限が先に当たり277.8万株＝発行済の5.56%、EPS 240円→254.1円（+5.88%）、出来高13.9日ぶん」を確認。
未実施: 本番デプロイ後の実データ表示確認（実例セクションはmicroCMSの記事に依存するためローカルでは出ない）。
...
デプロイAPIの実測（直近30日）: デプロイ682件（本番441・プレビュー241）、ビルド合計2,142分、
直近は1回8分（静的生成937ページに6.6分）。本番デプロイの43%（153/355件）はkujira-watch/に
変更の無いdocsコミット・keepalive空コミット。Elasticビルドマシンが9/2に「長時間ビルド」を理由に
Enhanced（8vCPU）へ自動昇格しており、$0.0035/CPU分×64CPU分＝1ビルド$0.22、月$130ペースだった。

Vercel側（APIで設定済み・コード外）:
- ビルドマシンをStandard固定、オンデマンド並列ビルドをOFF（Standard＋並列OFFはビルド課金なし）。

コード側:
- `vercel.json` の `ignoreCommand`（Ignored Build Step）: main以外はビルドしない＋Root Directory
  （kujira-watch/）に差分が無ければビルドしない。exit 0=スキップ、exit 1=ビルド。
  プレビュー241件と本番の43%（docsのみ・keepalive）が消え、デプロイは月682件→約200件になる見込み。
- 事前生成数を縮小: `/articles/[id]` 200→30、`/stocks/[code]` 300→30、`/investors/[filer]` 300→30、
  `/date/[date]` 60→7。事前生成外のページは初回アクセスでISR生成されCDNに載るので表示は変わらない。
- `@vercel/analytics` を撤去（Proでは無料枠なし$0.03/1Kイベントの従量課金。GA4が本番計測なので重複）。
  `@vercel/speed-insights` は無料枠のみで課金が無いので残す。プライバシーポリシーの表記も合わせた。
- README更新。
...
## 2026-08-30 分類色をOKLCHの単一パレットに統合し、アイキャッチの全角英数を是正

デザインシステム導入の続き。残っていた「デザイナーがいないように見える」2点を潰した。

- **アイキャッチの焼き込み文字**: EDINETの登録名が全角英数（`ＩＸＧＳ，　Ｉｎｃ．`）のまま画像に
  焼かれ、字間が間延びして読めなかった。`web/publish_blog_articles.py` に `display_text()` を追加し、
  提出者名・銘柄名を通してから焼き込む。`kujira-watch/src/lib/format.ts` の `displayText()` と同じ規則の写し
  （NFKCは使わない。全角括弧・句読点まで半角化して和文が崩れるため）。あわせて帯を黒(10,9,8)→
  ブランドネイビー(22,33,58)、バッジ文字を(255,200,80)→ブランドの金(217,164,79)に変更し、
  サイト上の注目カードと同じ配色にそろえた。テスト2件追加（計138件）。
- **分類色**: 投資家分類14種・保有目的5種・FAQカテゴリ9種が別々のパレットで、いずれもTailwind標準色から
  場当たりに選ばれていた（amber-600 / orange-500 / pink-500 / blue-500 / blue-600 …）。同じ役割の
  ドットなのに明度が段ごとにバラバラで、特定の分類だけが不自然に目立っていた。OKLCHで明度・彩度を
  そろえ色相だけを振り分けた単一パレットに統合（ドット L0.63/C0.15、文字 L0.46/C0.13）。文字色は
  クリーム地・紙面とも6.2:1以上。色相は意味のまとまり（暖色=個人/創業家/自社株買い、緑=事業/公益、
  青=銀行/国内運用、紫〜赤紫=海外運用）で割り当て、アクティビストだけ均等割りから外して彩度を上げた赤に
  した（最も注目される分類なので埋没させない）。FAQの `brand.gold` は文字色としてクリーム地で約3:1しか
  出ずAA未達だったので同時に解消。
- 分類色に薄い面を敷く箇所は16進アルファの連結（`${color}14`）だと中立色の `var(--ink-tertiary)` で
  壊れるため `color-mix()` に変更。
