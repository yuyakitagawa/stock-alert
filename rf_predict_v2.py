...
def compute_features(prices_arr):
    ...
    for i in range(90, n - FORECAST):
        ...
        # 過去SEQ_DAYS日の日次リターン系列（新機能: 価格パターンを直接学習）
        if len(p) >= SEQ_DAYS + 1:
            seq = np.diff(p[-(SEQ_DAYS + 1):]) / p[-(SEQ_DAYS + 1):-1]
            seq = np.clip(seq, -0.2, 0.2).tolist()  # 外れ値クリップ
        else:
            seq = [0.0] * SEQ_DAYS

        feat = [ret5, ret20, ret60, ret90, ma5_25, ma25_75, rsi, vol20, vol60, pos52] + seq
...
def main():
    print("=" * 60)
    print("XGBoost v2（生リターン系列追加）" + datetime.now().strftime("%Y-%m-%d %H:%M"))
    print(f"特徴量: テクニカル10個 + 過去{SEQ_DAYS}日リターン系列 = {10 + SEQ_DAYS}個")
    ...
    rise_path = os.path.expanduser("~/stock-alert/rf_model_v2.pkl")
    drop_path = os.path.expanduser("~/stock-alert/rf_drop_model_v2.pkl")
