...
「この投資家が大量保有報告書で買い増しを開示するたびに、その銘柄を1株買って
63営業日(3ヶ月)後まで保有していたら、株価はいくら動いたか」を投資家別に集計する。
...
def main():
    ...
    outcomes = []  # (filer_name, category, yen_return)
    immature = 0
    for e in events:
        ...
        outcomes.append((e["filer_name"], category, exit_close - entry_close))
    ...
    # 投資家別集計（1株買ったと仮定した円建てリターン）
    filer_stats = {}
    for filer, category, yen_return in outcomes:
        s = filer_stats.setdefault(filer, {"category": category, "n": 0, "returns": []})
        s["n"] += 1
        s["returns"].append(yen_return)
    ...
    for filer, s in filer_stats.items():
        if s["n"] < args.min_n:
            continue
        n = s["n"]
        total_return_yen = sum(s["returns"])
        avg_return_yen = total_return_yen / n
        rows.append({
            "filer_name": filer,
            "category": s["category"],
            "n": n,
            "total_return_yen": round(total_return_yen, 1),
            "avg_return_yen": round(avg_return_yen, 1),
            "hold_days": args.hold,
        })

    rows.sort(key=lambda r: -r["total_return_yen"])

    print(f"{'投資家':40s} {'分類':14s} {'n':>4s} {'1株あたり':>12s} {'トータル':>14s}")
    print("-" * 90)
    for r in rows:
        print(f"{r['filer_name'][:40]:40s} {r['category']:14s} {r['n']:>4d} "
              f"{r['avg_return_yen']:>+11.1f}円 {r['total_return_yen']:>+13.1f}円")
