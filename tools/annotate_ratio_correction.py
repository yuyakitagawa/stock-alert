...
注記が「他の株主との比較や順位、売買の方向の記述は訂正前の数値にもとづく場合がある」と断るのは、
...
売買の方向にも触れるのは、比率が直った結果「買い増し」と「売却」が入れ替わる記事が
実測28本あったため（2026-08-30）。本文1文目は決定的テンプレートなのでアンカーが明確で、
そこだけは方向語を直した（27本。1本はテンプレートに一致せず未修正）。本文の他の箇所の
方向語は文脈依存で機械的には直せないので触っていない。
...
def build_note(old_ratio: float, new_ratio: float) -> str:
    ...
    return (
        ...
        'なお本文中の、他の株主との比較や順位、売買の方向に関する記述は、'
        '訂正前の数値にもとづく場合があります。'
        '</p></aside>'
    )
