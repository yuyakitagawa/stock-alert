...
# 並び順が維持率を決める。2026-08-30の実測（8本）では3秒残存が91〜99%でhookは落ちておらず、
# 全8本が10秒地点で53〜75%まで落ちた。最急の落ち込みは実時間5〜9秒に集中していて、
# これは hook（約5.4秒）が終わって第2シーンに入る境目にあたる。そこに置いていたのが
# company（会社の事業内容）＝開示の続報ではなく背景説明だった。
# そこで「続報（誰が→前回からどう動いたか）」を前に、「背景説明（何の会社か）」を後ろに回す。
# deal はv7のhook（金額スラム→社名→動詞→提出者ラベル→保有比率）と内容が重複するため中盤へ。
# 2番目を change ではなく filer にしたのは、change は前回比率が取れない回にシーンごと落ちる
# （v7 rank8）ため、測りたい枠が回によって別のシーンになってしまうから。filer は実写背景が
# 付く2シーンの片方（background.VIDEO_BG_KINDS）でもあり、hook直後に絵が変わる。
SECTION_SPEC = [
...
{{"hook": {{"narration": "...", "caption": "..."}},
  "sections": [{{"kind": "filer", "narration": "...", "caption": "..."}}, ...4件...],
