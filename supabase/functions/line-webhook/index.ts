...
「最近の決算サプライズは？」「上方修正した銘柄」「ウォッチ銘柄のニュースは？」「大量保有報告」「機関投資家の動き」等にはcheck_catalystを使う。scope='watchlist'でウォッチリスト銘柄のみ、'all'で全銘柄。keyword='上方修正'等で絞込可能。🔥マーク付きはカタリスト候補（上方修正・増配・自社株買い等）。🏦マーク付きはEDINET大量保有報告（5%超保有）
