...
const TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    input_schema: {
      type: "object" as const,
      properties: {
        scope: { type: "string", description: "'watchlist'=ウォッチリスト銘柄のみ, 'all'=全銘柄から検索", enum: ["watchlist", "all"] },
...
async function executeCheckCatalyst(input: Record<string, unknown>, userId: string): Promise<string> {
  ...
  if (disclosures.length === 0) {
    const scopeStr = scope === "watchlist" ? "ウォッチリスト銘柄の" : "";
    const kwStr = keyword ? `「${keyword}」を含む` : "";
    return `直近${days}日間で${scopeStr}${kwStr}適時開示はありません。`;
  }
...
「最近の決算サプライズは？」「上方修正した銘柄」「ウォッチ銘柄のニュースは？」等にはcheck_catalystを使う。scope='watchlist'でウォッチリスト銘柄のみ、'all'で全銘柄。keyword='上方修正'等で絞込可能。🔥マーク付きはカタリスト候補（上方修正・増配・自社株買い等）

## ツール
銘柄を聞かれたら必ずlookup_stockで最新データを取得してから答える。ウォッチ操作は該当ツールを使う。条件検索にはscreen_stocks、適時開示にはcheck_catalystを使う。
