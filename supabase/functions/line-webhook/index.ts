...
async function saveMessage(userId: string, role: "user" | "assistant", content: string): Promise<void> {
  ...
  await fetch(`${SB_URL}/rest/v1/line_chat_history`, {
    method: "POST",
    headers: sbHeaders({ Prefer: "return=minimal" }),
    body: JSON.stringify({ line_user_id: userId, role, content: content.slice(0, 4000) }),
  });
}
...
大量保有・カタリスト系の質問は、直前に同じような質問に答えていても、会話履歴の記憶だけで答えず必ずcheck_catalystを毎回呼び直す（データが更新されている可能性があるため）。「さっき見せたものと同じです」「追加データはありません」のように、ツールを呼ばずに済ませるのは禁止。「もう一回見せて」「再度見せて」等の再表示依頼も同様に呼び直し、そのときのツール結果を省略せず列挙する
