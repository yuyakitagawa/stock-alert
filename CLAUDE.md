...
| ファイル | 役割 |
|---|---|
| utils.py | 共通関数（get_prices, extract_features, calc_rsi 等）、81次元特徴量定義 |
...
- 特徴量: 81次元（テクニカル10 + トレンド反転5 + 出来高3 + 日経マクロ3 + 60日リターン系列60）
- 予測期間: 63営業日（約3ヶ月）で±15%以上変動するかどうか
- AUC: 上昇 0.644 / 下落 0.780
...
- AUC 0.644はランダム（0.50）よりわずかに良い程度。参考指標として使い、最終判断は自分で行う。
