...
- **報告フォーマット（全報告に共通・例外なし）**: ユーザーへの報告は必ず次の3ブロックに分け、混ぜずに書け。作業完了報告・中間報告・調査結果・エラー報告のすべてに適用する。
  1. **依頼されたこと**: 何を頼まれたかの再掲。1〜2行。
  2. **ステータス**: 事実だけ。何がどこまで終わったか、数字・実行結果・検証結果。**できなかったこと・やらなかったこと・失敗も必ずここに書く**。推測や意見を混ぜない。
  3. **提案**: 次の一手・推薦・懸念。意見はこのブロックにだけ置く。無ければ「なし」と書く。
...
- **Anthropic API はActionsの日常作業のみ**: `ANTHROPIC_API_KEY` を使う処理を走らせてよいのは、GitHub Actions の日常ジョブ（毎日のランキング／記事生成／感情分析など既存ワークフロー内）だけ。ローカル・手動での API 実行（バックフィル、一括リライト、再生成、検証・調査目的の試し打ちを含む）は**オーナーの明示的な許可がない限り一切禁止**。「少量だから」「確認のため1件だけ」も禁止。必要な場合は実行せず、想定件数と概算コストを提示して許可を取れ。
- **API を増やす変更も同じ扱い**: 新たに Anthropic API を呼ぶコード・ツールを追加するときは、Actions の日常フローに組み込む形でのみ提案し、その場で走らせない。理由: 2026-08 に月次上限へ到達してサービスが停止した（重複課金のバックフィルが原因）。
