...
## 2026-09-06 記事の日次上限を撤回し、足切りを3億円/1.0ptへ戻した（更新頻度の回復）
オーナー判断「記事の更新が少ないとメディアとして微妙。前と同じ足切りで全ての記事を書く」。
2026-08-29の足切り引き上げ（3.0億円/1.0pt → 5.0億円/1.5pt）と、08-30の日次上限
（`DAILY_MAX_ARTICLES=2`）・便数削減（13便→3便）・日次予算の引き下げ（$1.2→$0.15）を撤回する。
どれもAPIコスト削減とA/Bの母集団固定が目的だったが、その代償が「メディアとしての更新頻度」だった。
- **日次上限を削除**。`DAILY_MAX_ARTICLES` / `daily_quota()` / `articles_published_today()` を
  publish_blog_articles・publish_buyback_articles の両方から消した（コメントアウトではなく削除）。
  通常運転は上限なしで、足切りを通った未記事化の開示をその便で全部投稿する。
  `--max-articles` の明示と `--backfill`（`BACKFILL_MAX_ARTICLES`=15件/便）の上限は残す。
- **足切りを 5.0億円/1.5pt → 3.0億円/1.0pt へ**。`INDEXABLE_MIN_*`（公開済み記事のindex基準）と
  同値に戻り、「出したのにnoindex」は引き続き起きない。読者に公開している数字なので
  `kujira-watch/src/lib/faqData.tsx` のFAQも同じコミットで直した。
- **edinet_blog.yml を3便→毎時13便**（`0 0-12 * * 1-5`）。上限が無ければ開示は提出の1時間以内に
  記事化される。減らした理由だった「枠を使い切った便の空回り」は上限と一緒に消えた
  （記事化する開示が無い便はClaudeを呼ばないのでAPI課金も増えない）。
- **日次予算 $0.15 → $1.2**。$0.15は実測で既に制約になっていた（2026-09-01〜09-04は毎日
  $0.15〜$0.18で上限に張り付き、記事は1日15〜19本で頭打ち＝backfill便15本でほぼ食い潰していた）。
  足切り3.0億円/1.0ptの通過は直近30日で1営業日あたり35〜40件（`edinet_large_holdings`）なので
  記事本文$0.0092×40本≒$0.37、会社説明（1社約$0.05）が数社乗っても$0.6には収まる。
- **月次上限（$15）のほうが先に当たる**点は変わらない。20営業日×$0.4〜0.6で月$8〜12となり、
  2026-08以前の定常（月約$13）に戻る計算。Anthropic側の月次上限を上げるかどうかはオーナー判断。
- テストは全件通過（test_publish_blog_articles 141→138件、test_publish_buyback_articles 22件）。
