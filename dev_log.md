...
HPは社ごとに構造が違って取りこぼすのに対し、有報は全社が同じタグで「事業の内容」「対処すべき課題」
「従業員の状況」を出しており、EDINET APIで無料・確実に取れる。Claude APIは使わない（要約しない）。
- `company_profile` テーブル（code主キー、事業の内容800字・課題400字・従業員数/平均年齢/平均年収/平均勤続）
- 通知は1銘柄2行→3行になり、3行目に事業の要点を40字で出す
- 見出し文の選び方: 「当社グループは…により構成されております」等の構成説明と「なお〜」の補足を飛ばし、
  事業・製造・販売・提供などの語を含む文を優先する。機械抽出なので完璧ではない（実測で15社中12社は妥当）

## 2026-09-20 決算データの欠落（2026-05-23〜06-28が0件）を修正し、2026-04-25以降を取り直し

`jquants_fin_summary` が4月末以降ほとんど更新されていなかった（FY行: 2025-05は2,143件→2026-05は59件、
2026-06は105件）。#325 の書類コード修正に加えて、取得の仕組み自体に2つの穴があった。

- **日次の取得窓が固定7日**: daily_alert Step 2d が毎日「直近7日」をまるごと取り直す作りで、
  ジョブが数日落ちるとその期間は二度と取りに行かない。2026-06-24〜07-09 は日次が連日
  failure/cancelled で、**2026-05-23〜06-28 の決算行が0件**。3月決算企業の有報提出ピーク（6月下旬）が
  丸ごと抜け、5801/5802/3110/5713 などは2025-05のFYが最新のままだった
- **保存が全日終了後に1回だけ**: 途中でジョブが止まるとその回の取得が全部消えていた

修正: 保存済みの `(code, disc_date)` は書類一覧の secCode で判定してXBRLを取りに行かない。
窓を広げても日次の取得は新着分だけで済むので `--days 7` → `--days 21` に（21日落ちても次の成功日に拾う）。
保存は日ごと。抽出ロジックを直したときのために `--force`（保存済みも取り直す）と `--end`（期間を区切る）を追加し、
backfill.yml に `edinet_end_date` / `edinet_force` を足した。

取り直し（2026-04-25〜09-19、Actions backfill.yml を2本に分割）: 有報2,599件・半期報975件のうち
**3,557/3,564件を保存**。2026-06のFY行 105→2,194件、2026-08の2Q行 0→540件。
未取得の5社は Canon(7751)・クレディセゾン(8253)・メディシノバ(4875)・YCP(9257) など米国基準/外国会社でタグが合わないもの。
あわせて訂正有価証券報告書(130)由来の134行（過去期の数値が当日開示として入り「最新決算」を上書きしていた）を削除。
削除前に同じ `fy_end` の行が他に残ることを全行で確認済み（バックアップはローカルに退避）。
検証: 4183三井化学 FY2026-03 売上1兆6,688億円・EPS91.62、5802住友電工 5兆1,102億円。
最新開示が `fy_end >= 2026-03` の銘柄は4,140中3,643。DBサイズ 372MB（Free上限500MB）。

**残る制約**: 1Q/3Qは四半期報告書の廃止（2024-04）でEDINETに存在しない（決算短信はTDnet側）。
会社予想 `fsales/fop/fnp` も有報・半期報には無い（2026-04-25以降の行は空）。
**予想は同日のうちに決算短信（TDnet XBRL）から取る経路を入れた**（下の「会社予想を決算短信（TDnet XBRL）から取り込む」）。
EPSサプライズ（`lib/fundamentals.py`）はこの期間は実質効かない。

## 2026-09-20 会社予想を決算短信（TDnet XBRL）から取り込む

J-Quants Free が2026-04-24分で提供終了し、以降の決算はEDINET（有報・半期報告書）由来で
会社予想が入らない。`jquants_fin_summary.fnp` は2026-04-25以降0件で、押し目買い通知の
「予想増益」を判定できない銘柄が増えていた（9/2の判定で除外47件→78件、候補22→11銘柄）。
決算短信のサマリーXBRLから予想（純利益・営業利益・売上）を取り、`doc_type='短信予想'`で保存する。
- 2026-08-14の389件の短信から342件（88%）で予想を取得。通期短信は来期予想を採る
- **TDnetはXBRLを約1か月で消す**ため、Apr〜Julの空白は埋められない（実測で5月分は404）。
  各社が次の四半期短信を出せば埋まる（3月期企業なら10月下旬〜11月の2Q短信）
- ついでに `lib/tdnet.py` の2つの誤りを修正: XBRLのURLを `document_url_xbrl` という
  存在しないキーで読んでいて常にNULL（正しくは `url_xbrl`）、カタリスト分類の
  キーワードが「決算short信」（短→shortの置換ミス）で決算短信を拾えていなかった

## 2026-09-19 押し目買い候補のLINE通知を追加（web/dip_buy_alert.py）

オーナーの方針「健全で将来性がある企業が一時的な要因で急落したときに買い、戻りを待つ」を対話で条件に落とした。
条件は 日経-2%以下 × 当日-4%以下 × 株価300円・売買代金1億円 × PBR<1.5 × 予想増益 × 従業員1000人以上、β順。
検証（詳細は docs/dip_buy_strategy.md）:
- 10年（Yahoo調整済み株価）で 日経-2%以下×-4%以下 は63日後平均+10.7%・勝率64%（全銘柄ランダム+2.5%）
- 個別要因の急落（市場が下げていない日の-5%）は勝率48%・大負け23%で、全銘柄ランダムより悪い → 市場連れ安だけに絞る
- 健全性（自己資本比率・ROE・営業CF）は効かず、PBR<1.5 と予想増益が大負けを減らす。β上位ほど戻りが大きい（11年中9年）
- 「一度買った銘柄は63日買い直さない」はオーナー判断で入れない（買うかは人間が決める）
実装で踏んだもの:
- `yahoo_price_cache` は過去行を分割調整しない。分割日の前日比（例: 古河電工 0.104=1:10、住友電工 0.268=1:4、2026-06-03）が
  βを歪め、分割前BPSとの比較でPBRが1/10になる。分割比の±8%以内を分割として検出し、BPSに比率を掛ける（±3%では取りこぼした）
- 2026-04-25以降の決算はEDINET由来で会社予想（fnp）が0件。同じ年度の予想と実績で増益判定しないようガードし、判定できなかった件数を本文に出す
- 従業員数は Supabase `company_employees` に保存（tools/fetch_employees.py、Step 2h で古い順に400銘柄/日）。当初は通知のたびにYahooへ問い合わせていたが、急落日に候補が数百あると遅く、並列取得の空振りで候補が漏れる


## 2026-09-19 決算データ（jquants_fin_summary）の取りこぼしと誤抽出を修正

4183三井化学の最新決算が 2026-02-05（3Q）のまま止まっていたのを調査。
- **2026-04-25〜06-28 が0件**: J-Quants Free は 4/24 分で終了、EDINET取得ステップの追加は 6/27（#54）。
  さらに 6/24〜7/2 は日次ジョブ自体が失敗/キャンセルで、「直近7日」の取得では3月決算企業の有報提出ピーク（6月下旬）を取り逃した
- **書類コードの対応が誤り**: 130 を四半期報告書、140 を半期報告書と誤認。実際は 130=訂正有価証券報告書、140=四半期報告書（2024-04廃止）、160=半期報告書。
  結果、半期報告書は1件も入らず、訂正報告書の過去期の数値が当日開示として保存されていた（6/27以降の396行中、決算期末から4か月超のFY行が69行・65銘柄）
- **IFRS会社で単体の数値を抽出**: 有報には日本基準の単体数値（NonConsolidatedMember）も載っており、タグの部分一致＋先勝ちで単体を拾っていた。
  4183 FY2026: 旧=売上7,498億円・EPS36.06（単体）→ 新=1兆6,688億円・EPS91.62（連結）

修正（`lib/edinet_financials.py`）: 取得対象を 120/160 に。連結（Memberなし）を優先し、連結の無い会社だけ単体。タグは完全一致。IFRSの親会社株主帰属利益・EPS・BPS・営業利益のタグを追加。
検証: J-Quants の同じ期の値と突き合わせ（±1%以内）。有報100件: 売上 90/96→93/95、営業益 87/94→91/94、純利益 97/100→97/100、EPS 92/100→98/100、BPS 93/100→98/100。
半期報60件: 売上 59/59→59/59、営業益 51/56→55/60、EPS 60/60→60/60。残る不一致は J-Quants の百万円丸めと保険会社など特殊な会社。
tests/test_edinet_financials.py（7件）を追加。
...
①減ったのは日本語記事ページの表示（日100〜230→30〜50）②Googlebot（Google IP帯）の巡回が8/15〜29の日約400件→8/30以降日約95件
③URL検査で8/10〜20作成の記事は12件中11件index済み、9/1〜11作成は12件中10件「検出-インデックス未登録」
④既存ページはfetch成功・sitemapエラー0＝ブロックではない ⑤過去3か月のクリック181件のうち38件(21%)を、8/18〜29に自分で消したURL（削除記事89・/en 182）が持っていた
⑥GSC生成AI機能の表示99回中95回が/en記事で、9/5〜10は1回に。引用46記事中36記事は英訳が残っているのに `/en/articles/:id` は日本語ページへ飛ばしていた。

変更:
- `lib/gsc_search_guard.py`（+テスト5件）: 記事を消す手動3ツールで、GSC直近90日に表示のあった記事を削除対象から外す。GSCが取れなければ削除を中止。実データで689記事が保護対象
- `kujira-watch/next.config.ts`: `/en`・`/en/articles/:id`・`/en/about`・`/en/privacy` を en.kujira-watch.com へ。英語版記事ページは英訳なし・削除済みなら日本語ページへ308（旧: 404）
- TOPの新着導線はSSRで30件のリンクが既に出ており、「検出-未登録」＝URLは発見済みなので変更しなかった

判定: SEOはバックテスト不可。1〜2週ごとに Googlebot巡回数（IP帯で絞る）・新規記事のURL検査12件・GSC生成AI機能の表示で前後比較する。

## 2026-09-11 AdSense「有用性の低いコンテンツ」3回目の洗い出し（B案: 記事の量は維持して型を崩す）

再審査は未申請。本番を実測し直し、原因を影響順に整理した（詳細は `docs/progress_adsense_round3.md`）。
①サイトマップ2,553URLのうち手書きの固定ページは39（1.5%）で、残りは開示ごとの自動生成
②index対象1,099本中1,089本が同じ直答文で始まり、1,093本が「※推測:」で終わる
③AI生成の明示＋匿名運営（YMYL）④英語版328URLが8/28で停止・本文中央値166語 ⑤数値の誤り。
オーナー判断は **B案**（SEO/GEOを落とさないよう記事数は維持し、型を崩して手書きの解説を足す）。

変更:

- `web/publish_blog_articles.py`: 定型の書き出しを禁止し、`pick_lead_angle()` が開示ごとに書き出しの切り口を選ぶ。
  報告書本文の保有目的・保有株数・取得資金・報告義務発生日をプロンプトへ渡す。「※推測:」は任意に。
  **新規記事にだけ効く**（既存1,099本の書き出しは未変更）。Anthropic API はActionsの日常便でのみ動く。
- `kujira-watch`: 読み方ガイド `/guides`（解説記事5本、`src/lib/guides.ts`）を新設。英語版に `/terms` `/contact`。
  銘柄ページの公開判定をサイトマップと同じ入力にそろえ、一時障害の404がISRに残らないようにした。
- `tools/fix_misreported_blog_articles.py`: 英語版（titleEn/bodyEn）も同時に直す。`tools/apply_rewritten_articles.py` は bodyEn/titleEn を受け付ける。

データの是正（microCMS/Supabase、全件バックアップ済み）:

- 和文31本の数値をEDINETに合わせた（`logs/fixed_articles_backup_20260910_144522.jsonl`）
- 英語版50本を是正後の事実で書き直した（Claude Codeが直接執筆、API不使用。`logs/en_rewrite_backup_20260911_b1〜b5.json`）
- 会社説明の誤り5社を修正（LIFULL=葬儀→不動産情報、日東紡=合成繊維→ガラス繊維、ジェイ・エス・ビー=医薬→学生マンション、チヨダ=油圧機器→靴小売、コシダカ=カラオケ館→まねきねこ）

未了（オーナー判断待ち）: 和文の文言誤り27本の修正案、英語版の残り32本、是正後に基準未満の44本とサカタのタネ1本の扱い、既存記事の書き出しの作り直し（B2）。
...
## 2026-08-29 削除済み記事の引き継ぎ先を「生存記事」までフォールバックさせる
- 実測: 登録した257件のうち、引き継ぎ先の銘柄ページが公開中（PR #290の条件を満たす）なのは
  131件＝51%だけ。検索表示のある削除済み記事124件では52件（表示265・クリック10/28日）しか
  引き継げず、残り72件（表示288・クリック10）は引き継ぎ先自体が404だった。
- 対応: 表示側 `resolveStockTarget()` で、銘柄ページが非公開なら**同じ銘柄の生存記事**へ振り替える。
  どちらも無ければリダイレクトせず404のまま（404へ308で送ると読者もクローラーも404に着地する）。
- 検証: `npx tsc --noEmit` / `npm run lint` 成功。

## 2026-08-29 生存記事へのフォールバックは該当0件だったので削除
- PR #294で入れた「引き継ぎ先の銘柄ページが非公開なら同じ銘柄の生存記事へ送る」を実測したところ、
  **該当0件**だった。銘柄ページが非公開になる条件（記事2件未満、または記事1件で会社説明なし）と、
  その銘柄の記事が削除されている状況が重なると、生存記事も残らないため。
  検索表示のある削除済み記事124件の行き先は「銘柄ページへ308が52件（表示265・クリック10/28日）／
  404のままが72件（表示288・クリック10）」で、後者に送り先は存在しない。
- 「念のため残す」は禁止（CLAUDE.md §7）なので`resolveStockTarget()`を削除し、
  `stockHref()`がnullなら404のままにする形に戻した。トップや一覧へ逃がす案は採らない
  （関連性の無いページへの301はソフト404として扱われ、評価が付かないどころか品質評価を下げる）。
- 本番検証（2026-08-29 21:0x）: `/articles/88eecs9gms`→308→`/stocks/4425`(200)、
  `i5thbqg32-7u`→`/stocks/7426`(200)、`ygciz55s0`→`/stocks/2997`(200)、`16rcuq_9e`→`/stocks/6573`(200)、
  引き継ぎ先の無い3件と未登録idは404を確認。

## 2026-08-30 YouTubeの視聴維持率を測れるようにし、動線をコメント欄へ寄せた
- 実測（`video/youtube_metrics.py --report`）: 公開9本・総再生6,909回・登録者5人。
  60秒以下5本が平均1,171回、60秒超4本が平均582回。GA4は14日でyoutube流入28セッション
  （エンゲージ32%）＝**再生に対して約0.5%**。届いてはいるが、サイトにも登録にも繋がっていない。
- **計測**: `video/youtube_analytics.py` を追加（YouTube Analytics API v2）。平均視聴率・
  平均視聴秒・登録者獲得数と、維持率カーブ（経過割合0.00〜1.00の101点）を取る。
  `survival_at()` が尺で割り戻して「先頭3秒の残存率」を本ごとに比較できる形にする。
  これまで持てていたのは再生数・高評価だけで、**どのシーンで落ちたかが分からないまま
  演出を変えていた**（v7の改修も効果を数値で言えていない）。
  Supabaseに `youtube_video_retention` と `youtube_videos` の4列（avg_view_pct /
  avg_view_sec / subscribers_gained / hook_survival）を追加。
- **導線**: 投稿直後に記事URLのコメントを自チャンネルから1件残す（`youtube_client.post_comment`）。
  Shortsの説明文は畳まれていて開かれないが、コメント欄は1タップで開く。
  `utm_source=youtube_comment` で説明文経由と分けて計測する。
  併せて説明文の1行目を、動画内字幕の再掲（hook）から「▼{銘柄名}の保有推移…」の誘導文に変更。
  CTAシーンの金色ピルを `kujira-watch.com` → `リンクはコメント欄に` にし、URLはその下に小さく残す。
  台本プロンプトの closing も「サイトで続きが読める」→「コメント欄のリンクから」に変更。
- **開通までに3段階詰まった**（いずれも同じ403に見えるが原因が別）: (1) `youtube_auth.py` が
  `.env` を読んでおらず起動しない → `load_dotenv` を追加。(2) 旧トークンのscopeがuploadのみ →
  オーナーが取り直して3つ付与。(3) GCPプロジェクト195678645751でYouTube Analytics APIが未有効
  → オーナーが有効化。さらに `dimensions=video` は `views`（または estimatedMinutesWatched）を
  含めてそれで降順ソートしないと400 `The query is not supported` になる。403の原因2種は
  本文で見分けて別々に案内するようにした。
- **初回の実測（2026-08-30、9本中8本。直近1本はAnalytics側の反映待ち）**:
  平均視聴率63.7%。3秒残存は**91〜99%でhookは落ちていない**（v7の100人コンサルが予測した
  「13秒でスワイプ」は外れ）。落ちているのは**5〜10秒**で、全8本が10秒地点で53〜75%まで下がる。
  最急の落ち込みは尺の7〜23%地点＝実時間5〜9秒に集中し、これは hook（約5.4秒）が終わって
  `company`（会社の事業内容）に入る境目にあたる。
  尺別: 45秒以下4本が10秒地点で56〜75%、66秒以上4本が53〜57%。
  突出は 8/21 横河電機（45秒・視聴169.6%＝ループ再生ぶんが乗る・10秒75%・30秒66%）。
- **やらなかったこと**: 上の結果を受けたシーン順の変更。第2シーンを company から何に替えるかは
  オーナー判断が要るため、データを提示して次の指示を待つ。
- **やらなかったこと**: 投稿の欠落対策（直近10平日のうち8/19・8/20・8/24の3日が未投稿）。
  オーナーの選択で今回の対象外。
...
## 2026-08-30 X投稿に「開示時点のPBR・ROE・配当性向」を足した（@26ooo対抗）
- 発端: オーナーから「@26ooo（じろ、29,072フォロワー。アクティビスト・エンゲージメント・
  資本コスト・非公開化）に勝てるように改善してほしい」。`tools/x_follow.py discover`が出した
  Tier A候補一覧の1件。
- 現状の実測（Supabase）: `x_followers`は8/22〜8/29まで**フォロワー0・フォロー0**、累計52投稿。
  直近45日の投稿34本の平均インプレッションは種別ごとに0.0〜2.0、最大でも5、
  プロフィールクリック・いいねは**全部0**。つまり負けているのは投稿の型ではなく配信先の数。
- できること/できないこと: 配信先を作る（フォロー・リプライ）のは手動運用側の仕事で、コードでは
  埋められない。コードで詰められるのは中身の差だけなので、@26ooo のテーマ（資本コスト）に
  こちらのデータで正面から乗る形にした。
- 実装: `web/x_insight.py`に`fetch_valuation_context()`/`build_valuation_line()`。記事投稿の
  3行目に「開示時点 PBR0.40倍・ROE-0.3%・配当性向41%」を出す。PBRは`gen_rankings`の開示日以前で
  最新の行、ROEは`jquants_fin_summary`の開示日以前で最新の**本決算(FY)**行の`np/equity`
  （四半期行の`np`は累計利益なのでROEが過小になる＝PIT規律）。`payout_ratio`は行によって
  比率(0.2)と%(20.0)が混在するため`lib/fundamentals`と同じ基準で正規化した。
- 収まらないときは「提出者の文脈」→「開示時点の数字」の順に落とす（後者のほうが
  「なぜこの銘柄が狙われたか」を運ぶため）。「割安」等の評価語は付けない
  （`docs/x_operation_rules.md` 2 に明記）。
- カバレッジ: 直近3日の実開示96件のうち**92件（96%）**で行が出た。PBRが取れないREIT等は
  ROE単独では資本コストの文脈にならないため行ごと出さない。
- コスト: 追加APIコストゼロ（Supabaseの既存テーブルを読むだけ。Anthropic APIは使わない）。
- テスト: `tests/test_x_client.py` を53→**62件**に。既存の投稿系テストが実ネットワークへ
  出ていたのも見つけて`fetch_valuation_context`をモックした（1.77s→0.55s）。
- 未実施（オーナー判断待ち）: Tier Aアカウントへのフォロー実行と、彼らの投稿に当方のデータで
  返すための下書き生成ツール。ローカル`.env`にX_API_KEYが無いため（Actionsのsecretsのみ）、
  X API を叩く検証はこのセッションではできていない。

## 2026-08-30 保有目的の判定範囲を広げた（直近553件で実測）
- 検証用に直近14日ぶん553件のXBRL本表を埋めて実測した。**保有目的は550/553件（99.5%）で取得**、
  平均取得単価が出るのは368件（67%。残りは取得資金の記載が無い開示か全部売却）、
  借入ありが89件・うち自己資金0の全額借入が6件、報告義務発生から30日超の遅延開示が51件、
  「重要提案行為等」欄に具体的な記載があるのが35件。
- 分類の判定不可が77件（14%）あり、中身は運用会社・証券会社の定型文（投資一任／投資信託／
  信託財産／ディーリング／トレーディング／一時保有）と「企業グループ間取引の維持強化」
  「経営の安定化を図るための継続保有」だった。これらを既存5区分に寄せて**判定不可を36件（6.5%）へ**。
- 寄せなかったもの: 「発行者との対話を行っている」「協議を活性化させる」といったエンゲージメント型
  （残り36件の一部）。重要提案行為等に入れると過剰主張になるため、区分を増やすかどうかはオーナー判断。
## 2026-08-30 同ジャンル9アカウント879投稿を実測（Xの型の天井が分かった）
- `tools/x_benchmark.py` を追加し、フォローした同ジャンル9アカウントの直近投稿879本を取得して
  中央値で比較した（平均は1本のバズで動くため使わない）。
- **開示を機械的に流す型は全滅**: @HoldingNavi 中央値eng 0 / imp 315、@eyeR365X 0 / 176、
  @avesan_27 0 / 274、@stpedia 0 / 2,284、@tairyo_hoyu 2 / 720。**これは今のkujira-watchと同じ型**で、
  5アカウントすべてが中央値0〜2に張り付いている。フォーマットを磨いてもこの天井は超えない。
- **桁が違うのは解釈を書いている2つだけ**: @kiokunir 中央値eng 69 / imp 23,678、@26ooo 13 / 4,428。
  事実の羅列ではなく「なぜそれが異常か」を一人称で書いている。
- 条件別（879本横断、交絡ありなので参考値）: 文字数は〜59字が eng 4.5 / imp 2,759 で最良、
  140字〜が 1.0 / 562 で最悪。**ハッシュタグは1個が最悪**（eng 0.0。0個と3個以上は2.0）。
  **URL有無は差なし**（eng 1.0 対 1.0）＝コスト削減で入れたURL無し方針は伸びを損なっていない。
  時間帯は朝7-11時が imp 2,080 で最良、現状の主戦場である夕16-19時は 710。
- 結論: 投稿フォーマットの改善は「同型の5アカウントが証明済みの天井（中央値0〜2）」までしか
  効かない。伸ばすなら型ではなく中身（解釈）を変えるしかない。声のトーンはブランド判断なので
  オーナー確認待ち。

## 2026-08-30 提出者ごとのリターン実績は不採用（オーナー判断）
- 「提出者ごとの3ヶ月後リターン実績」の投稿枠を実装したが、**公開しないと決めて撤去した**。
  理由は数字の不確実性。全開示平均が同じ日の実行でも +2.8%〜+6.4% で動き、原因を
  `yahoo_price_cache` への並行書き込みまで絞ったが確定できなかった。実在ファンドの成績を
  名指しで出す内容で、翌週に違う数字が出ると信頼を落とすため、出さない方を選んだ。
- 実装中に潰した不具合2件は、同じ書き方をすると再発するので記録しておく:
  ①`sb.select()` は自前で limit/offset を付けてページングするので、クエリ側にも `limit=` を
  書くと1つのURLに limit が2つ並び、終了条件に当たらず同じ行を重複取得する。
  ②丸める前の値で大小を判定すると「平均+3.6%・全開示平均+3.6%なので明確に下」のような
  表示と矛盾する文言が出る。判定は表示する丸めた値で行うこと。
- 集計自体で分かったこと（投稿はしないが事実として残す）: 証券会社の自己名義（野村證券255件など）を
  除外しないと「投資家の成績」にならない。旧字体の「證券」は「証券」では弾けない。
- 残したのは `tools/x_benchmark.py`（同ジャンル879投稿の実測）のみ。

## 2026-08-30 Xの投稿を「型」ではなく「材料」で変えた
- 日次サマリーを21時JST→**9時JST**へ移し、対象を前営業日にした。実測の中央値インプレッションが
  朝7-11時=2,080、夕16-19時=710、夜20-23時=1,094だったため。記事投稿の時間帯は絞っていない
  （午後公開の記事はその便でしか投稿機会が無く、窓を狭めると投稿自体が落ちる）。
- **開示原文にしか無い事実を出す枠**（`web/x_disclosure_facts.py`、火曜9時JST・不定期）を追加。
  全額借入での取得と30日超の遅延開示の2種類。**株価を使わないので騰落率の不確実性が入らない**
  （提出者ごとのリターン実績を撤去したのと同じ日に、その代わりとして作った）。
- 作りながら潰した誤報の芽3つ。いずれも「本表(XBRL)の解析率が低い」ことに起因する:
  ①「全額借入は今年4件」と書きかけたが、取得資金欄が埋まっているのは2026年の開示の**約2%**しか
  無く、直近30日だけで5件あった。解析済みの母数で希少性を語ってはいけない。
  ②分類マスターに載っていない個人名（蕪木 登など）が候補に出ていた。法人と分かる語を含まない
  提出者は個人とみなして除外する保守判定を入れた。
  ③保有比率3%へ下げた変更報告書を「買い増し、保有比率3%」と書いていた。変更報告書の取得資金欄は
  保有分全体の資金でその回の買い増し分ではないため、全額借入は新規報告書に限る。

## 2026-08-30 kujira-watchを「GEOのA/B実験台」に切り替えた（コスト削減＋母集団固定）
- **テストが本番Supabaseへ書いていた事故を止めた**。`tests/test_api_usage.py` の atexit flush が
  本番 `api_usage` に `job=local / task="x" / cache_write 1,000,000 / $1.35` の合成行を書き、
  その1行だけで当日合計が日次予算$1.2を超えて `check_daily_cap()` が誤発火する状態だった
  （＝翌営業日の記事生成が全便打ち切られる）。テスト側でモックを足す案は採らなかった。
  atexit は各テストの後片付けの外で走るので塞げず、新しいテストが増えるたびに漏れる。
  `lib/supabase_client.py` に `_block_production_write()` を置き、**テスト実行中かつ書き込み先が
  本番URLのとき**だけ upsert/insert_ignore/update/delete を握りつぶす（読み取りは通す。
  URLを差し替えているテストも通す）。全796件を流して `api_usage` が1行も増えないことを確認済み。
  合成行は `DELETE FROM api_usage WHERE job='local' AND task='x' AND usage_date='2026-08-29'` で削除。
- **記事は日2本**（`DAILY_MAX_ARTICLES=2`）。`daily_quota()` が当日(UTC)の `article_published_at` を
  数えて残り枠を出し、既存の `max_articles` にそのまま渡す（新しい概念を増やさない）。上限で
  見送った候補は台帳の `SKIP_MAX_ARTICLES`＝正常な見送りに落ちるのでLINEは鳴らない。
  「1便あたり」ではなく「1日あたり」にしたのは、A/Bの母集団が便数や開示件数で揺れると
  効果を測れないため。
- **edinet_blog.yml を13便→3便**（`0 0,6,9 * * 1-5`）。EDINETの提出は18時JST以降ゼロなので
  18時の便で当日分は出揃う。記事が日2本なら残りの便は空回りするだけ。backfill便は据え置き。
- **X投稿を全停止**。実測（08-19〜08-29）は34投稿・総インプレッション43・いいね0・リポスト0・
  URLクリック1・フォロワー10日連続0。`docs/x_operation_rules.md` が挙げるリーチの3経路
  （フォロワーTL／X内検索／他チャネル流入）が全部塞がっており、投稿の型を変えても届く先が無い。
  `x_post.yml` の6スケジュールと `publish_blog_articles.main()` の `post_top_articles` /
  `post_daily_summary` 呼び出しを削除（手動実行と `web/x_client.py` 本体は残す）。
  アカウントと既存投稿は消さない（「誤った投稿は削除しない」と同じ考え方）。再開条件は
  「サイトの自然検索が伸びてX以外からXへ人を送れる状態になったら」を x_operation_rules.md に明記。
- **video_post.yml の schedule を停止**（公開9本・総再生6,909回・登録者5人・コメント0）。コードは残す。
- **日次予算 $1.2 → $0.15**。記事本文1本の実測は $0.0092（api_usage、08-29）で2本なら約$0.02、
  未取得の会社説明2社ぶん（web_search込み1社約$0.05）を足しても約$0.12。暴走時の被害が1/8になる。
- **ハートビートからX・動画の監視を外した**。止めた成果物を数え続けると毎日「X投稿0件」で
  誤報になり、本当に見てほしい「記事0件」が埋もれる。

## 2026-08-31 X投稿は「完全停止」から「週1本の1週間まとめ」へ戻した
- 前日にXの定期投稿を全停止したが、XはサイトのOrganizationスキーマの`sameAs`と
  `contactPoint`（`kujira-watch/src/lib/site.ts`）に入っており、実名・メールを公開しない
  運営方針の下では**唯一の連絡窓口**だった。投稿ゼロのままだと `/about` の
  「速報は公式Xでも発信しています」が実態と食い違い、E-E-A-Tでは「アカウントが不活発なこと」
  よりも「運営者情報の記述が事実と違うこと」のほうが減点になる。
- 残したのは `x_post.yml` の土曜18:00 JST（`0 9 * * 6`）＝`web/x_weekly_trending.py` の
  **週次まとめ1本だけ**（直近7日 vs 前7日の急増ランキング）。記事1本ごとの投稿
  （`post_top_articles`）と日次サマリーは停止したまま。URL無し投稿は$0.015/本なので
  月約$0.06で、日次予算$0.15にも当たらない。
- `/about` の文言も「毎週土曜に大口投資家の取引急増ランキングを発信／質問・指摘も受け付け」へ
  書き換えた。発信内容を変えたらこの文言も対で直すこと（tsxにコメントで残した）。

## 2026-08-31 アイキャッチのURL拒否で記事が1本丸ごと落ちるのを止める
- `EDINET Blog Hourly` の run 33359138225 が「ブログ生成」で失敗（終了コード4）。原因は
  トヨクモ(4058) の投稿が microCMS から
  `HTTP 400 {"message":"'eyecatch' field invalid. Please set a valid URL."}` を返され、
  記事そのものが投稿されなかったこと。他の2本（数理技研616A・ＴＯＲＩＣＯ7138）は成功。
- これは既知の現象で、`tools/backfill_blog_eyecatch.py` の50件実行（2026-08-30）でも
  2件出ている。アップロード直後のメディアURLをコンテンツAPI側の検証が拒むもので、
  同じURLのまま数秒待って送り直すと通る。backfill側は `patch_eyecatch()` に
  リトライを入れて解決済みだったが、本番の投稿経路 `publish_article()` には無かった。
- `publish_article()` に `_INVALID_FIELD_RE` を追加。`'<field>' field invalid` の400は
  (1) 値を変えずに `MEDIA_PROPAGATION_WAIT`=3秒待って1回だけ再送信し、
  (2) それでも直らなければそのフィールドを外して再送信する。
  型不一致（`has unexpected data type`）の扱いと同じ思想で、**記事1本を落とすより
  画像なしで本文を出す**ほうを優先する。画像は作り直さない（同じ写真になるため）。
...
## 2026-09-09 新規の特例報告で推定取得金額が実態の十数倍になっていた
オーナー指摘「最近の金額あってる？高すぎる気がする」。原因は `ratio_change_pct()` が
**新規の大量保有報告書について、直前保有割合もDB履歴も無いとき今回の保有比率の全量を
「今回動いた分」として扱っていた**こと（`publish_blog_articles.py`）。変更報告書は既にNoneを
返して見送っていたが、新規報告書は非特例（TOB・資本業務提携）も特例報告も区別せず全量計上だった。
- **特例対象株券等（金商法27条の26）は機関投資家の2週間ごとの基準日報告**で、新規＝「基準日時点で
  5%を超えた」という事実しか示さない。直前が4.9%だったのか0%だったのかは開示から読み取れない。
- 実測（2026-08-09〜09-08、`edinet_large_holdings`）。全量計上になった開示は106件、うち記事化80件:
  - 新規・特例: 44件中40件公開。公開分の推定金額 合計15,697億円 / 平均402.5億円
  - 新規・非特例: 62件中40件公開。合計3,247億円 / 平均90.2億円
  - 参考: 変更報告書（実変化幅で計算）の平均は28.9億円（非特例）/54.9億円（特例）
  → 新規・特例だけ平均が7〜14倍に膨らんでいた。
- 最悪例: 任天堂(7974) 2026-09-07、キャピタル・リサーチが5.2%で新規の特例報告。
  時価総額11.26兆円×5.2% = **5,857億円を「推定取得金額」として公開**。直前4.9%なら実際の買いは
  0.3pt＝338億円で17.3倍の過大表示。
- DB上、同じ発行体×提出者で「新規」報告書が2回以上出ているペアが267組あり、5%ラインを
  出入りしているだけで全量取得ではないことが裏付けられる。
- 直し: `is_special_report()`（`doc_description`に「特例」を含むか）を追加し、`ratio_change_pct()`に
  `is_special`を渡す。**履歴が無い新規の特例報告は変化幅Noneを返し**、呼び出し側が
  `SKIP_NO_PRIOR_RATIO`で記事化を見送る。非特例の新規届出（実際に全量を取得する開示）は従来どおり。
  特例でも直前比率やDB履歴があれば差分で計算するので影響しない。
- 影響: 記事が直近30日ペースで40本／30日＝約1.3本/日減る。金額が原理的に確定できない開示なので
  記事の一次価値も低い。記事にしない開示は従来どおり `/date/[date]` の「この日の対象開示」に並ぶ。
- テスト: test_publish_blog_articles 149→152件（特例判定・履歴無し新規特例のNone・非特例は全量のまま・
  直前比率/履歴があれば差分）。既存テストの `ratio_change_pct` モック4か所を新シグネチャへ追随。
- 既存テスト全件を実行。失敗は `test_video_pipeline.py::test_thumbnail_compose_writes_1280x720_png`
  のみで、これは本修正前（stash して確認）でも同じく失敗する日本語フォント未導入によるもの。
- 未実施: 全量計上のまま公開済みの80本（うち特例40本）の後追い対応。ラベル変更か非公開かは未決。
