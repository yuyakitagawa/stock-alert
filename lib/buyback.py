...
# 「変更・訂正・中止」: 既存の取得枠に対する後追いの開示。新規決議ではない。
# _DECISION_RE の「取得に係る事項」が
#   ・「自己株式取得に係る事項の一部変更に関するお知らせ」（8560 2026-08-18）
#   ・「（訂正）「自己株式取得に係る事項の決定に関するお知らせ」の一部訂正について」（4323 2026-06-30）
# の両方にマッチするため、これを先に弾かないと決定として記事化される。8560は取締役会決議が
# 2026-02-09なのに「2026-08-18に決議した」という誤報になる（閾値も超えるので実際に危なかった）。
# 中止・撤回はニュース価値自体はあるが、新規決議前提の本文テンプレートで出してはいけない。
_AMENDMENT_RE = re.compile(r"変更|訂正|中止|取りやめ|取止め|撤回")
...
def classify_buyback_title(title: str) -> str:
    """'amendment' / 'progress' / 'decision' / 'other'。

    決定語より先に変更・訂正・中止と進捗を判定する。決定語が混ざっていても、
    「取得結果及び取得終了並びに消却」は進捗、「…決定に関するお知らせの一部訂正」は訂正であり、
    どちらも新規の取得枠決議ではないため。記事化の対象は 'decision' だけ。"""
    t = title or ""
    if _AMENDMENT_RE.search(t):
        return "amendment"
