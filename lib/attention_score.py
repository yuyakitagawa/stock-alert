...
実際の63営業日後リターンとは弱い負の相関〜無相関だった（tools/filer_win_rate.pyと同じ
バックテスト手法で買い開示4,232件を検証、2026-08-15）。そのため直感ではなく実績データで
較正したスコアカード方式を採用する。
...
再学習: サンプル・分布が古くなったら同じ手法（tools/filer_win_rate.pyのイベント抽出＋
本ファイルのビン集計）で定数を再計算し、下記の定数を差し替える。
"""
import math
