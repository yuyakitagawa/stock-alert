...
# 2026-09-06に記事の日次上限を撤廃した（足切りを通った開示は全部記事にする）のに合わせて
# 0.15 → 1.2 へ戻した。0.15のままだと足切り通過ぶんを出し切る前に打ち切られる
# （実測: 2026-09-01〜09-04は毎日$0.15〜$0.18で上限に張り付き、記事は1日15〜19本で頭打ち）。
# 根拠: 記事本文1本の実測は $0.0092（api_usage、2026-08-29）。足切り3.0億円/1.0ptの通過は
# 直近30日で1営業日あたり35〜40件（edinet_large_holdings、2026-09-06）なので約$0.37。
# 未取得の会社説明（web_search込みで1社約$0.05）が数社ぶん乗っても$0.6には収まる。
# 月次上限（DEFAULT_MONTHLY_BUDGET_USD=15）のほうが先に当たるので、月末の打ち切りは
# 日次ではなく月次の通知（_ALERT_LEVELS）で気付く運用。
