...
## 2026-08-22 追記: 「検出 - インデックス未登録」4,223件の正体とクロール速度の改善

ユーザーからGSCのカバレッジ画面と、同画面からエクスポートしたCSV3点
（`表.csv`=未登録URL1,000件サンプル・`平均読み込み時間のチャート.csv`=日次推移）を受けて調査。

### 結論: 品質判定ではなく「未クロール」
- `表.csv` の1,000件は **全件が前回クロール日 `1970-01-01`（=一度もクロールされていない）**。
  Googleは中身を見ておらず、品質の判断は下されていない。
- 品質で落とされた「クロール済み - インデックス未登録」は **62件のみ**（登録済み1,914件に対し誤差の範囲）。
- 日次推移: 08-05 71 → 08-08 644 → 08-11 906 → **08-15 4,223**。8/15の `8f95386d`
  （投資家1,900件のサイトマップ漏れ修正、URL 1,713→6,196）と一致。内訳は
  `/investors` 603件(60%)・`/en` 230件(23%)・`/articles` 71・`/date` 46・`/stocks` 42。
- チャートは08-17で止まっており、8/18〜19の対策（薄い投資家ページ982件のsitemap除外・
  低価値記事129本削除・投資家ページ404焼き付き修正）より前のスナップショット。
  突合すると未登録1,000件のうち今もsitemapに載っているのは247件（`/investors`は0件）。

### 残っていた実害: コールド応答が遅くクロール速度を律速
Googlebot UAでキャッシュ未温のURLを実測: `/investors/*` 1.4〜3.5秒・`/articles/*` 0.5〜3.5秒・
`/stocks/*` 0.5〜2.3秒、いずれも `x-vercel-cache: MISS`。`revalidate=300` に対し事前生成は
100件だけで、sitemapの投資家1,968件・銘柄1,292件のほぼ全てが初回アクセスで再生成に当たる。
`/en/articles/[id]`・`/en/stocks/[code]` は `generateStaticParams` が無く毎回SSR。

### 実装（2026-08-22）
- [x] `revalidate` 300→86400: `/investors/[filer]`・`/stocks/[code]`・`/en/stocks/[code]`
      （データ源は日次更新）。注意: microCMS fetchの `revalidate: 60` が route の実効値を
      下げるため、ビルド表示上 `/stocks` は `1m` のまま。ただしstale-while-revalidateで
      クローラーにはキャッシュが即返るので、クロール速度の観点では事前生成＋ISR化が本質。
- [x] `/en/articles/[id]`（英訳済み直近100件）・`/en/stocks/[code]`（英訳記事のある100銘柄）に
      `generateStaticParams` を追加 → ビルド出力が `ƒ`(Dynamic) から `●`(SSG/ISR) に変化。
- [x] 事前生成 100→300: `PRERENDERED_FILERS`・`PRERENDERED_STOCKS`。
- [x] microCMSクライアントに `retry: true`（SDK標準の429/5xx指数バックオフ）。事前生成が
      1,148ページになりビルドが `429 Too many requests` で1回失敗したため必須。
      再ビルドで成功を確認（この変更とREADME更新は並行セッションのコミット `fc3bf24b` に同梱された）。

効果の確認はGSC反映待ち（[[feedback_seo_indexing_wait]]）。
