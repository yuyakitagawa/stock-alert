...
- 既存の説明表示: `StockLivePanel`「この会社について」→ `/api/stock/[code]/description`
  → Supabase `gen_ai_analyses`(model_version=`company-desc-v1`, date=`1970-01-01`) をキャッシュ参照。
- スプシ専用シート **「📝 会社説明」**（コード/銘柄名/説明）を新設し手動管理（自動上書きされない）。
- `web/sync_descriptions.py`: シートを読み、説明が入っている行を `gen_ai_analyses` の company-desc-v1 へ upsert。
