...
- 2026-09-06: **ステップ2・3・6を撤回**（オーナー判断「記事の更新が少ないとメディアとして
  微妙。前と同じ足切りで全ての記事を書く」）。日次上限 `DAILY_MAX_ARTICLES`/`daily_quota()` を
  削除し、足切りを2026-08-29以前の 3.0億円/1.0pt へ戻し、`edinet_blog.yml` を毎時13便へ、
  日次予算を $1.2 へ戻した。実測でも $0.15 は制約になっていた（2026-09-01〜09-04は毎日
  $0.15〜$0.18で上限に張り付き、記事は1日15〜19本で頭打ち）。
  母集団固定を前提にしたGEOのA/B設計は、**日ごとの本数ではなく記事の書き分け**で組む必要がある。
  ステップ1（本番書き込みガード）・4（X週1本）・5（動画停止）・7（ハートビート）は維持。
