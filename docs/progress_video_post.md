...
## v8改修（2026-08-30 オーナー指示「youtube投稿の改善をしたい」→ 導線・計測・中身の3方向）
実測を先に取った: 公開9本 / 総再生6,909回 / 登録者5人 / コメント0件。
60秒以下5本が平均1,171回、60秒超4本が平均582回。GA4は14日でyoutube流入28セッション
（再生に対し約0.5%、エンゲージ32%）。

- [x] 視聴維持率の計測基盤（`video/youtube_analytics.py`）。平均視聴率・平均視聴秒・
      登録者獲得数＋維持率カーブを YouTube Analytics API v2 から取り、`youtube_video_retention`
      と `youtube_videos` の4列に保存。`--report` に平均視聴率とhook3秒の残存率を表示
- [x] 導線をコメント欄へ。投稿直後に記事URLのコメントを自動投稿（`utm_source=youtube_comment`）
- [x] 説明文の1行目を hook の再掲から「▼{銘柄名}の保有推移・提出者の全開示はこちら」に
- [x] CTAの金色ピルを `kujira-watch.com` → `リンクはコメント欄に`（URLは下段に小さく残す）。
      台本プロンプトの closing も同じ行き先に統一
...
|---|---|---|---|---|---|---|---|
| 08-16 | 89s | 1,288 | 27.8% | 99% | 59% | 34% | 25% |
| 08-17 | 66s | 177 | 34.2% | 91% | 53% | 36% | 28% |
| 08-18 | 75s | 342 | 31.6% | 98% | 55% | 32% | 25% |
| 08-18 | 84s | 520 | 39.1% | 95% | 57% | 37% | 35% |
| 08-21 | 45s | 1,201 | 169.6% | 98% | 75% | 70% | 66% |
| 08-25 | 39s | 1,354 | 76.4% | 91% | 61% | 51% | 42% |
| 08-26 | 40s | 1,198 | 75.5% | 94% | 64% | 52% | 44% |
| 08-27 | 45s | 938 | 55.3% | 98% | 56% | 35% | 31% |

- **hookは落ちていない**（3秒残存91〜99%）。v7で採用した「100人中70人がhookで離脱」という
  パネルの予測は実測では外れていた。
- **落ちるのは5〜10秒**。最急の落ち込みは尺の7〜23%地点＝実時間5〜9秒で全8本が一致し、
  これは hook（約5.4秒）が終わって `company`（会社の事業内容）に入る境目。
- 視聴率が100%を超えるのはループ再生ぶん。`survival_at()` はカーブ先頭で正規化して比べる。

### 今回やらなかったこと
- 投稿の欠落対策。直近10平日のうち 8/19・8/20・8/24 の3日が未投稿（8/24はAnthropic API上限）。
  オーナーの選択で対象外にした。
- コメントのピン留め。YouTube Data API に機能が無いため、必要ならStudioで手動。
