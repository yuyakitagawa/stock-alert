...
- ROE / BPS = `kabutan_fundamentals`、自己資本比率・equity = `jquants_fin_summary`。
...
- `app/api/stock/[code]/insight/route.ts`: Claude Haiku で 事業概要(拡張)/主要取引先/カタリスト評価/リスク を生成・Supabase(`claude_ai_analyses`, model_version `company-insight-v1`)キャッシュ。
