...
export default function StockCard({ r }: { r: Ranking }) {
  const s = signalStyle(r.recommend);
  return (
    <Link
      href={`/stocks/${r.code}`}
      className={`group block rounded-xl border ${s.border} bg-gray-900 hover:bg-gray-800 transition-colors p-4 space-y-3`}
    >
      {/* Header */}
